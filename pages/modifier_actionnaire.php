...
if (isset($_GET['id'])) {
 $getid=$_GET['id'];
 $select=$bdd->prepare('SELECT * FROM actionnaires WHERE id=?');
           ...
           if ( isset($_POST['nom'])
             ...
             AND isset($_POST['montant_souscrit'])
          ...
          
                                {
                                                       ...
                                                       $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
                                                       $req=$bdd->prepare('UPDATE actionnaires SET nom=? WHERE id=?');
                                                       $req->execute(array($nom,$getid));
                                                       header('Location:index.php?page=actionnaires&id='.$getid);
                                                       $req=$bdd->prepare('UPDATE actionnaires SET prenom=? WHERE id=?');
                                                       $req->execute(array($prenom,$getid));
                                                       header('Location:index.php?page=actionnaires&id='.$getid);
                                                      
                                                       $req=$bdd->prepare('UPDATE actionnaires SET telephone=? WHERE id=?');
                                                       $req->execute(array($telephone,$getid));
                                                       header('Location:index.php?page=actionnaires&id='.$getid);

                                                       $req=$bdd->prepare('UPDATE actionnaires SET adresse=? WHERE id=?');
                                                       $req->execute(array($adresse,$getid));
                                                       header('Location:index.php?page=actionnaires&id='.$getid);
                                                       $req=$bdd->prepare('UPDATE actionnaires SET montant_souscrit=? WHERE id=?');
                                                       $req->execute(array($montant_souscrit,$getid));
                                                       header('Location:index.php?page=actionnaires&id='.$getid);
                                 ...
                                 if (isset($_FILES['monfichier']) AND $_FILES['monfichier']['error'] == 0)
                                {
                                        ...
                                        if ($_FILES['monfichier']['size'] <= 1000000)
                                        {
                                                ...
                                                if (in_array($extension_upload, $extensions_autorisees))
                                                {
                                                        ...
                                                        $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
                                                        $req=$bdd->prepare('UPDATE actionnaires SET logo=? WHERE id=?');
...
<?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $select=$bdd->prepare('SELECT * FROM actionnaires where id=? ORDER BY id DESC');
...
<?php
  ...
  <path fill-rule="evenodd" d="M3 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1H3zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/>
   ...
   <a style="font-size:1.2rem; color:#A41FDE" class="nav-link  " href="index.php?page=modifier_actionnaire&id=<?=$resultat['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" d="M13.498.795l.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
                </svg> Modification du compte de l'Actionnaire <?= $donnees['nom'] ?></a>
     ...
     <div class="container">
        <div class="col-lg-8">
                            ...
                            <div class="card-body">
                                ...
                                Modification du compte de <?= $donnees['nom'] ?>
                                </h1><hr>
                                <div class="row">
                              ...
                              <label for="inputZip">Nom</label>
                                <input type="text" value='<?= $donnees['nom'] ?>'  name='nom'class="form-control mr-5" id="inputCity" required placeholder="SIDIBE">
                              </div>
                              </div>
                              <div class="form-row">
                                  <div class="col-12"></div>
                              <div class="form-group col-12">
                              <label for="inputZip">Prenom</label>
                                <input type="text" value='<?= $donnees['prenom'] ?>'  name='prenom'class="form-control mr-5" id="inputCity" required placeholder="KARIM ">
                              </div>
                              </div>
                              ...
                              <label for="inputZip">Photo de l'actionnaire</label>
                              ...
                              <div class="form-row">
                                  <div class="col-12"></div>
                              <div class="form-group col-12">
                              <label for="inputZip">Montant souscrit</label>
                                <input type="number" value='<?= $donnees['montant_souscrit'] ?>'  name='montant_souscrit'class="form-control mr-5" id="inputCity" required placeholder="EX: 200 000">
                              </div>
                              </div>
