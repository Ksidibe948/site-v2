...
?>
<?php
 $req=$bdd->prepare('SELECT * FROM    CessionImmoFinanciere WHERE id=?');
...
?>
 <?php 
       ...
       if (isset($_POST['marchandise'])
        ...
        $ventess = $bdd->prepare('UPDATE   CessionImmoFinanciere set quantite= ? where id= ? ');
        $ventess->execute(array($nouvelle_valeur, $_GET['id']));
      
        $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');

        $req=$bdd->prepare('UPDATE   CessionImmoFinanciere SET marchandise=? WHERE id=?');
        $req->execute(array($marchandise,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoFinanciere&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE   CessionImmoFinanciere SET id_client=? WHERE id=?');
        $req->execute(array($numero_client,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoFinanciere&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE   CessionImmoFinanciere SET quantite=? WHERE id=?');
        $req->execute(array($quantite,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoFinanciere&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE   CessionImmoFinanciere SET prix=? WHERE id=?');
        $req->execute(array($prix,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoFinanciere&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE   CessionImmoFinanciere SET taux_tva=? WHERE id=?');
        $req->execute(array($taux_tva,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoFinanciere&id='.$_GET['id']);
                                         
        
        $req=$bdd->prepare('UPDATE   CessionImmoFinanciere SET monyen_payement=? WHERE id=?');
        $req->execute(array($condition_payement,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoFinanciere&id='.$_GET['id']);
             
        $req=$bdd->prepare('UPDATE   CessionImmoFinanciere SET montant_payement=? WHERE id=?');
...
$bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   ...
   ?>
        <?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $vente_marchandise=$bdd->prepare('SELECT * FROM   CessionImmoFinanciere where id_entreprise=? ORDER BY id DESC');
...
<div class="row">
 ...
 <a style=' font-size:1.1rem;;' class="nav-link text-success p-3  mb-1 shadow-sm bg-light " href="visteurs.php?page=ModifierFacatureCessionImmoFinanciere&id=<?=$reponse['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M13.498.795l.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
   ...
   <div class="col-lg-6 col-12"> 
                            ...
                            <div class="card-body">
                                <h1 class="card-title text-light bg-secondary text-center">
                            ...
                            <div class="form-group row">
                              <label for="inputPassword" class="col-sm-2 col-form-label"><h6>Immo : </h6> </label>
                              <div class="col-sm-3">
