...
<?php
 if (isset($_GET['id'])) 
 {
  ...
  $select=$bdd->prepare('SELECT* FROM   AchatsImmoIncorporelle  WHERE id=?');
       ...
       ) 
       {
        $getid=$_GET['id'];
         ...
         $modification->execute(array($modifications_bq,$condition_payement ));
          // ModificationStockImmobilisationIncoporelle
          $reqs=$bdd->prepare('UPDATE  stocksimmoincorporelle  SET marchandiseS=? WHERE id=?');
          $reqs->execute(array($marchandise,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);
          
          $reqs=$bdd->prepare('UPDATE  stocksimmoincorporelle  SET id_clientS=? WHERE id=?');
          $reqs->execute(array($numero_client,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);
          
          $reqs=$bdd->prepare('UPDATE  stocksimmoincorporelle  SET quantiteS=? WHERE id=?');
          $reqs->execute(array($quantite,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);
          
          $reqs=$bdd->prepare('UPDATE  stocksimmoincorporelle  SET prixS=? WHERE id=?');
          $reqs->execute(array($prix,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);
             
                                 
          $reqs=$bdd->prepare('UPDATE  stocksimmoincorporelle  SET taux_tvaS=? WHERE id=?');
          $reqs->execute(array($taux_tva,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);
          
          $reqs=$bdd->prepare('UPDATE  stocksimmoincorporelle  SET monyen_payementS=? WHERE id=?');
          $reqs->execute(array($condition_payement,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);
               
          $reqs=$bdd->prepare('UPDATE stocksimmoincorporelle  SET montant_payementS=? WHERE id=?');
          $reqs->execute(array($montant_payer,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);
                    
          $reqs=$bdd->prepare('UPDATE  stocksimmoincorporelle SET systemeS=? WHERE id=?');
          $reqs->execute(array($systeme,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);
                              
          $reqs=$bdd->prepare('UPDATE  stocksimmoincorporelle  SET dureeS=? WHERE id=?');
          $reqs->execute(array($duree,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);

                            
          $reqs=$bdd->prepare('UPDATE  stocksimmoincorporelle  SET moisS=? WHERE id=?');
          $reqs->execute(array($mois,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);
          // ModificationImmobilisationIncoporelle
          $req=$bdd->prepare('UPDATE  AchatsImmoIncorporelle  SET marchandise=? WHERE id=?');
          $req->execute(array($marchandise,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE  AchatsImmoIncorporelle  SET id_client=? WHERE id=?');
          $req->execute(array($numero_client,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE  AchatsImmoIncorporelle  SET quantite=? WHERE id=?');
          $req->execute(array($quantite,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE  AchatsImmoIncorporelle  SET prix=? WHERE id=?');
          $req->execute(array($prix,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);
             
                                 
          $req=$bdd->prepare('UPDATE  AchatsImmoIncorporelle  SET taux_tva=? WHERE id=?');
          $req->execute(array($taux_tva,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE  AchatsImmoIncorporelle  SET monyen_payement=? WHERE id=?');
          $req->execute(array($condition_payement,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);
               
          $req=$bdd->prepare('UPDATE  AchatsImmoIncorporelle  SET montant_payement=? WHERE id=?');
          $req->execute(array($montant_payer,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);
                    
          $req=$bdd->prepare('UPDATE  AchatsImmoIncorporelle  SET systeme=? WHERE id=?');
          $req->execute(array($systeme,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);
                              
          $req=$bdd->prepare('UPDATE  AchatsImmoIncorporelle  SET duree=? WHERE id=?');
          $req->execute(array($duree,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoIcorporelle&id='.$_GET['id']);

                            
          $req=$bdd->prepare('UPDATE  AchatsImmoIncorporelle  SET mois=? WHERE id=?');
...
$bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
 ...
 $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   ...
   ?>
     <?php
$bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $stocks_marteriels=$bdd->prepare('SELECT * FROM AchatsImmoIncorporelle  where id_entreprise=? ORDER BY id DESC');
...
<?php
  ...
  <div class="row">
    ...
    <a  class="nav-link p-3  mb-1 shadow-sm bg-light "  style="font-size:1.2rem;color:#A41FDE" href="visteurs.php?page=AchatsImmobilisationsIncorporelles&id=<?=$_SESSION['id'] ?>"> 
  <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
   <path fill-rule="evenodd" d="M14.5 3h-13a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
   <path fill-rule="evenodd" d="M5 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 5 8zm0-2.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zm0 5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5z"/>
   <circle cx="3.5" cy="5.5" r=".5"/>
 ...
 <a style="font-size:1.2rem;" class="nav-link p-3  mb-1 shadow-sm bg-light text-success" href="visteurs.php?page=modifierFAImmoIncorporelle&id=<?=$_GET['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path fill-rule="evenodd" d="M13.498.795l.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
     ...
     <div class="col-lg-6 col-12"> 
                         <!-- la partie inscription -->
                            ...
                            <div class="card-body">
                                <hr><h1 class="card-title text-center p-4 text-light bg-secondary">
                               Nouvelle facture Achat  <?= $donnees['marchandise'] ?>
                                </h1><hr>
