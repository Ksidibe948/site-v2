...
<?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $CessionImmoIncorporelle=$bdd->prepare("SELECT
       CessionImmoIncorporelle.id,
       CessionImmoIncorporelle.id_client,
       CessionImmoIncorporelle. id_entreprise,
       CessionImmoIncorporelle.marchandise,
       CessionImmoIncorporelle.quantite,
       CessionImmoIncorporelle.prix,
       CessionImmoIncorporelle.taux_tva,
       CessionImmoIncorporelle. monyen_payement,
       CessionImmoIncorporelle. montant_payement,
        clients.nom,
        clients.telephone,
        clients. adresse,
        clients.logo,
        DATE_FORMAT(CessionImmoIncorporelle.date,'%d/%m/%Y à %Hh%imin%ss') AS date
   FROM  CessionImmoIncorporelle,clients WHERE  CessionImmoIncorporelle.id_client=clients.id AND  CessionImmoIncorporelle. id=? ORDER BY id DESC");
   $CessionImmoIncorporelle->execute(array($_GET['id']));
   $compte=  $CessionImmoIncorporelle->rowcount();
   $donnees=$CessionImmoIncorporelle->fetch();
 ?>
...
<div class="row">
 ...
 <a style="font-size:1.2rem; color:orange; font-family:arial" class="nav-link p-3  mb-1 shadow-sm bg-light " href="visteurs.php?page=JournalFactureCessionImmoIncorporelle&id=<?=$_GET['id'] ?>"><svg width="1em" height="1.5em" viewBox="0 0 16 16" class="bi bi-book" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
