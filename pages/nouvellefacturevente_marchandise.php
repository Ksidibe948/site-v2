...
?>
<?php
 $req=$bdd->prepare('SELECT * FROM stoks_marchandise WHERE id=?');
 $req->execute(array($getid));
 $reponse=$req->fetch();
...
?>
 <?php 
       ...
       if (isset($_POST['marchandise'])
        ...
        $ventes = $bdd->prepare('UPDATE stoks_marchandise set quantiteS=quantiteS - ? where id= ? ');
        $ventes->execute(array($quantite, $_GET['id']));
      
      $insert=$bdd->prepare('INSERT INTO ventes
      (
        id_client,
        id_entreprise,
        marchandise,
    ...
    header('Location:visteurs.php?page=fature_vente_marchandise&id='.$getid);
