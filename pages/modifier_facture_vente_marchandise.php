...
?>
<?php
 $req=$bdd->prepare('SELECT * FROM ventes WHERE id=?');
 $req->execute(array($getid));
 $reponse=$req->fetch();
...
?>
 <?php 
       ...
       if (isset($_POST['marchandise'])
        ...
        $ventes = $bdd->prepare('UPDATE ventes set quantite= ? where id= ? ');
        $ventes->execute(array($nouvelle_valeur, $_GET['id']));
      
        $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');

        $req=$bdd->prepare('UPDATE ventes SET marchandise=? WHERE id=?');
        $req->execute(array($marchandise,$getid));
        header('Location:visteurs.php?page=detail_vente_marchandise&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE ventes SET id_client=? WHERE id=?');
        $req->execute(array($numero_client,$getid));
        header('Location:visteurs.php?page=detail_vente_marchandise&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE ventes SET quantite=? WHERE id=?');
        $req->execute(array($quantite,$getid));
        header('Location:visteurs.php?page=detail_vente_marchandise&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE ventes SET prix=? WHERE id=?');
        $req->execute(array($prix,$getid));
        header('Location:visteurs.php?page=detail_vente_marchandise&id='.$_GET['id']);
             
        $req=$bdd->prepare('UPDATE ventes SET reduction=? WHERE id=?');
        $req->execute(array($reduction,$getid));
        header('Location:visteurs.php?page=detail_vente_marchandise&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE ventes SET taux_reduction=? WHERE id=?');
        $req->execute(array($taux_reduction,$getid));
        header('Location:visteurs.php?page=detail_vente_marchandise&id='.$_GET['id']);
                     
        $req=$bdd->prepare('UPDATE ventes SET taux_exompte=? WHERE id=?');
        $req->execute(array($taux_exompte,$getid));
        header('Location:visteurs.php?page=detail_vente_marchandise&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE ventes SET montant_transport=? WHERE id=?');
        $req->execute(array($montant_transport,$getid));
        header('Location:visteurs.php?page=detail_vente_marchandise&id='.$_GET['id']);
                               
        $req=$bdd->prepare('UPDATE ventes SET taux_tva=? WHERE id=?');
        $req->execute(array($taux_tva,$getid));
        header('Location:visteurs.php?page=detail_vente_marchandise&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE ventes SET montant_emballage=? WHERE id=?');
        $req->execute(array($montant_emballage,$getid));
        header('Location:visteurs.php?page=detail_vente_marchandise&id='.$_GET['id']);
                                         
        $req=$bdd->prepare('UPDATE ventes SET montant_avance=? WHERE id=?');
        $req->execute(array($montant_avance,$getid));
        header('Location:visteurs.php?page=detail_vente_marchandise&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE ventes SET monyen_payement=? WHERE id=?');
        $req->execute(array($condition_payement,$getid));
        header('Location:visteurs.php?page=detail_vente_marchandise&id='.$_GET['id']);
             
        $req=$bdd->prepare('UPDATE ventes SET montant_payement=? WHERE id=?');
...
<div class="row">
   ...
   <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    ...
    Factures
     ...
     <a  class="nav-link p-3  mb-1 shadow-sm bg-light bg-light bg-light " style="font-size:1.3rem;color:#A41FDE"  href="visteurs.php?page=vente_marchandise&id=<?=$_SESSION['id'] ?>">              
   <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
     <path fill-rule="evenodd" d="M14.5 3h-13a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
     <path fill-rule="evenodd" d="M5 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 5 8zm0-2.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zm0 5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5z"/>
     <circle cx="3.5" cy="5.5" r=".5"/>
    ...
    Factures de Ventes Marchandises
     </a>
 <a style=' font-size:1.1rem;;' class="nav-link text-success p-3  mb-1 shadow-sm bg-light  " href="visteurs.php?page=modifier_facture_vente_marchandise&id=<?=$reponse['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M13.498.795l.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
   ...
   <div class="col-lg-6 col-12">  
                            ...
                            <div class="card-body">
                              <h1 class="card-title p-4 text-light bg-secondary  text-center">
                              Modifier la Facture de Vente <?= $reponse['marchandise']?>
                                </h1>
