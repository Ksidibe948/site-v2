...
if (isset ($_GET['id']))
{
   $get=$_GET['id'];
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
    $req=$bdd->prepare("SELECT publications.fichier,  publications.id_publications, publications.id_utilisateurs, publications.contenu,utilisateurs.nom,utilisateurs.email, utilisateurs.images ,DATE_FORMAT(date,  '%d/%m/%Y à %Hh%imin%ss') AS date FROM publications, utilisateurs WHERE publications.id_utilisateurs=utilisateurs.id AND id_utilisateurs=? ORDER BY publications.date DESC");
...
if (isset ($_GET['id']))
{
   $get=$_GET['id'];
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
    $infos=$bdd->prepare("SELECT publications.fichier,  publications.id_publications, publications.id_utilisateurs, publications.contenu,utilisateurs.nom,utilisateurs.email, utilisateurs.images ,DATE_FORMAT(date,  '%d/%m/%Y à %Hh%imin%ss') AS date FROM publications, utilisateurs WHERE publications.id_utilisateurs=utilisateurs.id AND id_utilisateurs=? ORDER BY publications.date DESC");
    $infos->execute(array($get));
    $info=$infos->fetch();
...
if (isset ($_GET['id']))
{
   $get=$_GET['id'];
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
    $utilisateurs=$bdd->prepare("SELECT publications.id_publications, publications.id_utilisateurs, publications.contenu,utilisateurs.nom,utilisateurs.email, utilisateurs.images ,DATE_FORMAT(date,  '%d/%m/%Y à %Hh%imin%ss') AS date FROM publications, utilisateurs WHERE publications.id_utilisateurs=utilisateurs.id AND id_utilisateurs=?");
    $utilisateurs->execute(array($get));
    $info_utilisateurs=$utilisateurs->fetch();
...
<div class="row">
  ...
  <div class="collapse navbar-collapse" id="navbarNavDropdown">
     ...
     <a href='#' class="nav-link active "style="font-size:1.2rem; color:#A41FDE" ><svg width="1.2rem" height="1.2rem" viewBox="0 0 16 16" class="bi bi-calculator-fill" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
     <path fill-rule="evenodd" d="M2 2a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V2zm2 .5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 .5.5v2a.5.5 0 0 1-.5.5h-7a.5.5 0 0 1-.5-.5v-2zm0 4a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1zM4.5 9a.5.5 0 0 0-.5.5v1a.5.5 0 0 0 .5.5h1a.5.5 0 0 0 .5-.5v-1a.5.5 0 0 0-.5-.5h-1zM4 12.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1zM7.5 6a.5.5 0 0 0-.5.5v1a.5.5 0 0 0 .5.5h1a.5.5 0 0 0 .5-.5v-1a.5.5 0 0 0-.5-.5h-1zM7 9.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1zm.5 2.5a.5.5 0 0 0-.5.5v1a.5.5 0 0 0 .5.5h1a.5.5 0 0 0 .5-.5v-1a.5.5 0 0 0-.5-.5h-1zM10 6.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1zm.5 2.5a.5.5 0 0 0-.5.5v4a.5.5 0 0 0 .5.5h1a.5.5 0 0 0 .5-.5v-4a.5.5 0 0 0-.5-.5h-1z"/>
     </svg>Publications
     </a>
     <a class="nav-link text-dark  "style="font-size:1.2rem;" href="index.php?page=profil&id=<?=$_SESSION['id'] ?>"><svg width="1.2em" height="1.2em" viewBox="0 0 16 16" class="bi bi-person" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" d="M10 5a2 2 0 1 1-4 0 2 2 0 0 1 4 0zM8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm6 5c0 1-1 1-1 1H3s-1 0-1-1 1-4 6-4 6 3 6 4zm-1-.004c-.001-.246-.154-.986-.832-1.664C11.516 10.68 10.289 10 8 10c-2.29 0-3.516.68-4.168 1.332-.678.678-.83 1.418-.832 1.664h10z"/>
          </svg> Profil
        </a>
        <a class="nav-link active " style="font-size:1.2rem;" href="index.php?page=photos_utilisateurs&id=<?=$info['id_utilisateurs'] ?>"><svg width="1.2em" height="1.2em" viewBox="0 0 16 16" class="bi bi-person" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" d="M10 5a2 2 0 1 1-4 0 2 2 0 0 1 4 0zM8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm6 5c0 1-1 1-1 1H3s-1 0-1-1 1-4 6-4 6 3 6 4zm-1-.004c-.001-.246-.154-.986-.832-1.664C11.516 10.68 10.289 10 8 10c-2.29 0-3.516.68-4.168 1.332-.678.678-.83 1.418-.832 1.664h10z"/>
          </svg> Photos</a>
    </li>
</ul>
 ...
 <div class="container">
  <div class="col-lg-9">     
         <div class="row">
          ...
          { ?>
             <div class="col-lg-6">
             <a class="nav-link text-dark" href="index.php?page=les_articles&id=<?=$result['id_publications'] ?>"> 
             <div class="card mb-3 shadow-lg  bg-white" style="max-width:600px;">
                  <div class="card-title pt-2 pb-2 mt-2 mr-1 ml-1 bg-light pl-4" style=' border-radius: 50px;'>
                  <?="<img src='".$result['images']."' class='images' style='width:1.5rem ; height:1.6rem; border-radius:100px ; '  alt=''>" ;?> 
                   <?=$result['nom'] ?>
                  </div>
                  <div class="card-body text-left"> 
                   <?=$result['date'] ?>
                  <p class="card-text "><?= substr($result['contenu'],0,45) ?><span style="color:blue"></span></p>
                  <div class='imagess'>
                  <?="<img src='".$result['fichier']."' class='card-img-top  pt-2 pb-2  mr-2 mt-2'   alt=''>" ;?> 
                  </div>
                 </div>
                 </div>
