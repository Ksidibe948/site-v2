...
<?php
   ...
   FROM achats_immobilisation,fournisseurs WHERE achats_immobilisation.id_client=fournisseurs.id AND achats_immobilisation.id=? ORDER BY id DESC");
   $achat_imo->execute(array( $_GET['id']));
   $donnees=$achat_imo->fetch();
 ?>
...
Facture de <?= $donnees['marchandise'] ?>
</a>
<a style="font-size:1.2rem;" class="nav-link text-success" href="index.php?page=modifier_achat_immobilisation&id=<?=$_GET['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
           ...
           <a  style="font-size:1.2rem;" class="nav-link text-danger" href="index.php?page=supprimer_achat_immobilisation&id=<?=$_GET['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-x" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
