...
<?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $karim=$bdd->prepare('SELECT * FROM banque where id_entreprise id=? ORDER BY id DESC');
   $karim->execute(array($_SESSION['id']));
   while($karim=$karim->fetch()) {
     echo $karim['montant'];
  }
...
<?php
           if (
             isset($_POST['apport'])
             AND isset($_POST['numero_bancaire'])
...
<?php
  ...
  <path fill-rule="evenodd" d="M3 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1H3zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/>
   ...
   <a class="nav-linkpl-5 " style="font-size:1.2rem;color:#A41FDE"   href="index.php?page=liberation_action&id=<?=$resultat['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-book" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
               ...
               </svg> Libération d'une actions</a>
  ...
  <a  class="nav-link " style="font-size:1.2rem;" href="index.php?page=Bilan&id=<?=$_SESSION['id'] ?>">
   ...
   <div class="col-lg-9">
                            ...
                            <div class="card-body">
                              ...
                              <div class="col-12">
                              <input type="text"  name='apport'class="form-control" id="inputCity "  placeholder="numeraire, nature, industrie ">
                              </div>
                            </div><hr>
                            <div class="form-group row">
                              <label for="inputPassword" class="col-12 col-form-label"> Banque:  </label>
                              <div class="col-12">
                              <select name="numero_bancaire" id="" class="form-control">
                              <?php
