...
if (isset($_GET['id'])) {
   ...
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $select=$bdd->prepare('SELECT * FROM    charges_noncourantes WHERE id=? ORDER BY id DESC');
...
<?php
       ...
       {   
            ...
            $req=$bdd->prepare('UPDATE    charges_noncourantes SET nom_operation=? WHERE id=?');
            $req->execute(array($nom_operation,$getid));
            header('Location:visteurs.php?page= detail_chargesexceptionnelles&id='.$getid);
            $req=$bdd->prepare('UPDATE    charges_noncourantes SET montant_th=? WHERE id=?');
            $req->execute(array($montant_th,$getid));
            header('Location:visteurs.php?page= detail_chargesexceptionnelles&id='.$getid);
            $req=$bdd->prepare('UPDATE    charges_noncourantes SET taux_tva=? WHERE id=?');
            $req->execute(array($taux_tva,$getid));
            header('Location:visteurs.php?page= detail_chargesexceptionnelles&id='.$getid);
            $req=$bdd->prepare('UPDATE    charges_noncourantes SET numero_banque=? WHERE id=?');
            $req->execute(array($numero_banque,$getid));
            header('Location:visteurs.php?page=detail_chargesexceptionnelles&id='.$getid);
            $req=$bdd->prepare('UPDATE    charges_noncourantes SET montant_payer=? WHERE id=?');
