...
?>
   ...
   $ACHATSS=$bdd->prepare("SELECT
        ...
        achats. monyen_payement,
        achats. montant_payement,
      fournisseurs.nom,
      fournisseurs.telephone,
      fournisseurs. adresse,
      fournisseurs.logo,
        DATE_FORMAT(  achats.dates,'%d/%m/%Y à %Hh%imin%ss') AS date
   FROM achats,fournisseurs WHERE achats.id_fournisseur=fournisseurs.id AND  achats.id_fournisseur=? ORDER BY id DESC");
   $ACHATSS->execute(array( $_GET['id']));
 ?>
     
     <?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $achat_imo=$bdd->prepare("SELECT
        ...
        fournisseurs. adresse,
        fournisseurs.logo,
        DATE_FORMAT(achats_immobilisation.date,'%d/%m/%Y à %Hh%imin%ss') AS date
   FROM achats_immobilisation,fournisseurs WHERE achats_immobilisation.id_client=fournisseurs.id AND achats_immobilisation.id_client=? ORDER BY id DESC");
   $achat_imo->execute(array( $_GET['id']));
 ?>
  <?php
  $compptemat=$bdd->prepare('SELECT * FROM achats WHERE id_fournisseur=?');
  $compptemat->execute(array(($_GET['id'])));
  $compptemats=$compptemat->rowcount();
 ?>
 <?php
  $compptemateriel=$bdd->prepare('SELECT * FROM achats_immobilisation WHERE id_client=?');
 ...
 ?>
   <?php
    $_GET['id'];
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $select=$bdd->prepare("SELECT id, id_entreprise,nom, telephone,adresse, logo,DATE_FORMAT(date,'%d/%m/%Y à %Hh%imin%ss') AS date FROM fournisseurs where id=? ORDER BY id DESC");
...
<?php
  ...
  <a  class="nav-link  " style="font-size:1.2rem; color:#A41FDE"  href="#">
   ...
   <a style="font-size:1.2rem;" class="nav-link  text-success" href="index.php?page=modifier_fournisseur&id=<?=$resultat['id']?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" d="M13.498.795l.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
                </svg> Modifier Client <?= $resultat['nom'] ?></a>
                <a  style="font-size:1.2rem;" class="nav-link  text-danger" href="index.php?page=supprimer_fournisseur&id=<?=$resultat['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-x" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
               <path fill-rule="evenodd" d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708z"/>
               </svg> Supprimer Client <?= $resultat['nom'] ?></a>
   <a style='font-size:1.1rem' class="nav-link  text-info"  href="index.php?page=nouveau_fournisseur&id=<?=$_SESSION['id'] ?>">
  ...
  <a  class="nav-link " style="font-size:1.2rem;" href="index.php?page=Bilan&id=<?=$_SESSION['id'] ?>">
   ...
   while($donnees= $ACHATSS->fetch()){
       ...
       <div class="container">
        <div class="row mb-2">
         <div class="col-lg-12">
         <a  class="nav-link text-dark "  href="index.php?page=achat&id=<?=$donnees['id'] ?>">
            ...
            <div class="row ml-2 mr-2 ">
              <div class="col-lg-12">
                <div class="row">
                  ...
                  <div class="col-6 col-xl-6  text-center" style='border:solid black 0.5px'>
                    <h6>Elements</h6>
                  </div>
                  ...
                  <?php
                   if (isset($donnees['taux_escompte'])) {
                     ?>
                        </div>
                  <div class="row">
                  <div class="col-6 col-xl-6 " style='border:solid black 0.5px'>
   ...
   while($donnees=$achat_imo->fetch()){
       ...
       <div class="container">
        <div class="row mb-2">
         <div class="col-lg-12">
         <a  class="nav-link text-dark "  href="index.php?page=detail_materiel&id=<?=$donnees['id'] ?>">
         <div class="card w-100 shadow-lg p-3 bg-white" >
         <hr><div class="row">
            <div class="col-lg-12 text-right">
           <?= $donnees['date'] ?>
