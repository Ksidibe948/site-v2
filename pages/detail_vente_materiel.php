<?php
  ...
  ?>
    ...
    <?php if (isset($_GET['id'])) {
       $getid=$_GET['id'];
       $vente=$bdd->prepare('SELECT * FROM vente_materiels WHERE id_entreprise=?');
       $vente->execute(array($getid));
        $count=$vente->rowcount();
        
      
    }?>
<?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $vtm_materiels=$bdd->prepare("SELECT
         vente_materiels.id,
         vente_materiels.id_client,
         vente_materiels. id_entreprise,
         vente_materiels.marchandise,
         vente_materiels.quantite,
         vente_materiels.prix,
         vente_materiels.reduction,
         vente_materiels.taux_reduction,
         vente_materiels.taux_exompte,
         vente_materiels.montant_transport,
         vente_materiels.taux_tva,
         vente_materiels. montant_emballage,
         vente_materiels. montant_avance,
         vente_materiels. monyen_payement,
         vente_materiels. montant_payement,
        clients.nom,
        clients.logo,
        clients.telephone,
        clients. adresse,
        DATE_FORMAT(vente_materiels.date,'%d/%m/%Y à %Hh%imin%ss') AS date
    
   FROM    vente_materiels,clients WHERE    vente_materiels.id_client=clients.id AND  vente_materiels. id=? ORDER BY id DESC");
  $vtm_materiels->execute(array($_GET['id']));
  $donnees=$vtm_materiels->fetch();
...
<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
 ...
 Factures Ventes de materiels <span style='color:green'>( <?=$vente_materiels?>)</span> 
  ...
  <a  class="nav-link  " style="font-size:1.1rem; color:#A41FDE"  href="index.php?page=ventes_materiels&id=<?=$_SESSION['id'] ?>">              
<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M14.5 3h-13a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
  <path fill-rule="evenodd" d="M5 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 5 8zm0-2.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zm0 5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5z"/>
  <circle cx="3.5" cy="5.5" r=".5"/>
 ...
 Factures Ventes de materiels 
  </a>
  <a class="nav-link text-success " style='font-size:1.1rem' href="index.php?page=modifier_facture_vente_materiel&id=<?=$donnees['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
   <path fill-rule="evenodd" d="M13.498.795l.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
   </svg> Modifier la facture de <?= $donnees['marchandise'] ?>
    </a>
   <a  class="nav-link " style='font-size:1.1rem; color:orange' href="index.php?page=journal_facture_vente_materiel&id=<?=$donnees['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-book" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  ...
  </svg> Journal de la facture de <?= $donnees['marchandise'] ?></a>
   <a class="nav-link text-danger " style='font-size:1.1rem' href="index.php?page=supprimer_fature_vente_materiel&id=<?=$donnees['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-x" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708z"/>
  </svg> Supprimer la facture de <?= $donnees['marchandise'] ?></a>
...
<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
 ...
 Factures Ventes de marchandises<?php
      if ($vente_marchandise>0) {
        ?> <span style='color:green'><?= $vente_marchandise?></span> 
  </a><hr>
  </a>
 ...
 Factures de autres produits   <?php
   if ($autres_produits>0) {
     ?> <span style='color:green'> ( <?=$autres_produits?>)</span>
    <?php
   }
   ?>
  </a><hr>
...
<?php
      if ($stok_marchandise>0) {
        ?> <span style='color:green'>( <?=$stok_marchandise?> )</span> 
        </a><hr>
  ...
  ?>
    ...
    Stocks des Materiels  <?php
      if ($stok_materiel>0) {
        ?> <span style='color:green'> ( <?= $stok_materiel?>)</span>
    </a><hr>
     <?php
      }
  ?>
...
<?php
       ...
       <div class="container">
        <div class="row mb-2">
         <div class="col-12">
          <div class="card shadow-lg mt-5 w-100"><hr>
             <div class="row mr-5">
            ...
            <hr>
            <div class="row mr-5">
             <div class="col-12 ml-2 text-right">
             <em>Client</em><br>
