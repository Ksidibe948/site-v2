...
?>
<?php
 $req=$bdd->prepare('SELECT * FROM vente_materiels WHERE id=?');
 $req->execute(array($getid));
 $reponse=$req->fetch();
...
?>
 <?php 
       ...
       if (isset($_POST['marchandise'])
        ...
        $nouvelle_valeur = $reponse['quantite'] - $quantite ; 
        $vente_materiels = $bdd->prepare('UPDATE vente_materiels set quantite= ? where id= ? ');
        $vente_materiels->execute(array($nouvelle_valeur, $_GET['id']));
      
        $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');

        $req=$bdd->prepare('UPDATE vente_materiels SET marchandise=? WHERE id=?');
        $req->execute(array($marchandise,$getid));
        header('Location:index.php?page=detail_vente_materiel&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE vente_materiels SET id_client=? WHERE id=?');
        $req->execute(array($numero_client,$getid));
        header('Location:index.php?page=detail_vente_materiel&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE vente_materiels SET quantite=? WHERE id=?');
        $req->execute(array($quantite,$getid));
        header('Location:index.php?page=detail_vente_materiel&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE vente_materiels SET prix=? WHERE id=?');
        $req->execute(array($prix,$getid));
        header('Location:index.php?page=detail_vente_materiel&id='.$_GET['id']);
             
        $req=$bdd->prepare('UPDATE vente_materiels SET reduction=? WHERE id=?');
        $req->execute(array($reduction,$getid));
        header('Location:index.php?page=detail_vente_materiel&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE vente_materiels SET taux_reduction=? WHERE id=?');
        $req->execute(array($taux_reduction,$getid));
        header('Location:index.php?page=detail_vente_materiel&id='.$_GET['id']);
                     
        $req=$bdd->prepare('UPDATE vente_materiels SET taux_exompte=? WHERE id=?');
        $req->execute(array($taux_exompte,$getid));
        header('Location:index.php?page=detail_vente_materiel&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE vente_materiels SET montant_transport=? WHERE id=?');
        $req->execute(array($montant_transport,$getid));
        header('Location:index.php?page=detail_vente_materiel&id='.$_GET['id']);
                               
        $req=$bdd->prepare('UPDATE vente_materiels SET taux_tva=? WHERE id=?');
        $req->execute(array($taux_tva,$getid));
        header('Location:index.php?page=detail_vente_materiel&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE vente_materiels SET montant_emballage=? WHERE id=?');
        $req->execute(array($montant_emballage,$getid));
        header('Location:index.php?page=detail_vente_materiel&id='.$_GET['id']);
                                         
        $req=$bdd->prepare('UPDATE vente_materiels SET montant_avance=? WHERE id=?');
        $req->execute(array($montant_avance,$getid));
        header('Location:index.php?page=detail_vente_materiel&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE vente_materiels SET monyen_payement=? WHERE id=?');
        $req->execute(array($condition_payement,$getid));
        header('Location:index.php?page=detail_vente_materiel&id='.$_GET['id']);
             
        $req=$bdd->prepare('UPDATE vente_materiels SET montant_payement=? WHERE id=?');
...
$bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
  ...
  <?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $vente_materiels=$bdd->prepare('SELECT * FROM vente_materiels where id_entreprise=? ORDER BY id DESC');
...
<a  class="nav-link  " style="font-size:1.1rem; "  href="index.php?page=achats&id=<?=$_SESSION['id'] ?>">              
<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M14.5 3h-13a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
  <path fill-rule="evenodd" d="M5 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 5 8zm0-2.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zm0 5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5z"/>
  <circle cx="3.5" cy="5.5" r=".5"/>
 ...
 Factures Achats de Marchandises <span style='color:green'> ( <?=$stok_marchandise?> )</span>
  ...
  <?php
      if ($stok_materiel>0) {
        ?>
<a  class="nav-link  " style="font-size:1rem; " href="index.php?page=achat_immobilisation&id=<?=$_SESSION['id'] ?>">
<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M14.5 3h-13a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
  <path fill-rule="evenodd" d="M5 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 5 8zm0-2.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zm0 5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5z"/>
  <circle cx="3.5" cy="5.5" r=".5"/>
...
<a  class="nav-link  " style="font-size:1.1rem;"  href="index.php?page=ventes_materiels&id=<?=$_SESSION['id'] ?>">              
<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M14.5 3h-13a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
  <path fill-rule="evenodd" d="M5 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 5 8zm0-2.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zm0 5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5z"/>
  <circle cx="3.5" cy="5.5" r=".5"/>
 ...
 Factures Ventes de materiels <span style='color:green'>( <?=$vente_materiels?>)</span> 
  ...
  <a class="nav-link  " style='font-size:1.3rem ;color:#A41FDE' href="index.php?page=modifier_facture_vente_materiel&id=<?=$reponse['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
   <path fill-rule="evenodd" d="M13.498.795l.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
   </svg> Modification de  la facture de <?= $reponse['marchandise']?>
    </a>

  <a style='font-size:1.3rem' class="nav-link  text-info "  href="index.php?page=Liste_vente_materiels&id=<?=$_SESSION['id'] ?>">
  <svg width="1.1em" height="1.1em" viewBox="0 0 16 16" class="bi bi-plus" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path fill-rule="evenodd" d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z"/>
  </svg>
     Ajouter une nouvelle <br> facture de vente marchandise
  </a>
  <hr>
<?php
...
<?php
      if ($stok_marchandise>0) {
        ?>
       <a  class="nav-link   " style="font-size:1.1rem;"href="index.php?page=stocks_marchandise&id=<?=$_SESSION['id'] ?>"> 
        <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-cart3" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        <path fill-rule="evenodd" d="M0 1.5A.5.5 0 0 1 .5 1H2a.5.5 0 0 1 .485.379L2.89 3H14.5a.5.5 0 0 1 .49.598l-1 5a.5.5 0 0 1-.465.401l-9.397.472L4.415 11H13a.5.5 0 0 1 0 1H4a.5.5 0 0 1-.491-.408L2.01 3.607 1.61 2H.5a.5.5 0 0 1-.5-.5zM3.102 4l.84 4.479 9.144-.459L13.89 4H3.102zM5 12a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm7 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm-7 1a1 1 0 1 0 0 2 1 1 0 0 0 0-2zm7 0a1 1 0 1 0 0 2 1 1 0 0 0 0-2z"/>
       </svg>
 ...
 <?php
      if ($stok_materiel>0) {
        ?>
        <a  class="nav-link  " style="font-size:1.1rem;" href="index.php?page=stocks_materiels&id=<?=$_SESSION['id'] ?>"> 
        <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-cart3" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
      <path fill-rule="evenodd" d="M0 1.5A.5.5 0 0 1 .5 1H2a.5.5 0 0 1 .485.379L2.89 3H14.5a.5.5 0 0 1 .49.598l-1 5a.5.5 0 0 1-.465.401l-9.397.472L4.415 11H13a.5.5 0 0 1 0 1H4a.5.5 0 0 1-.491-.408L2.01 3.607 1.61 2H.5a.5.5 0 0 1-.5-.5zM3.102 4l.84 4.479 9.144-.459L13.89 4H3.102zM5 12a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm7 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm-7 1a1 1 0 1 0 0 2 1 1 0 0 0 0-2zm7 0a1 1 0 1 0 0 2 1 1 0 0 0 0-2z"/>
    </svg>
...
<div class="col-lg-8 col-12  mt-5  "> 
                            ...
                            <div class="col-lg-12  ">
                            <div class="card shadow-lg mt-5 bg-light w-100" style="width: 18rem; ">
                            <div class="card-body">
                                <h1 class="card-title text-center">
                              Modifier facture vente de  <?= $reponse['marchandise']?>
                                </h1><hr>
                                <form action="" method="POST" enctype="multipart/form-data">
                              <div class="form-group row">
                              <label for="inputPassword" class="col-sm-3 col-form-label">Numéro Client :  </label>
                              <div class="col-sm-3">
                              <select id=""  name='numero_client' class="form-control">
                                <?php
                                    $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
                                    $numero_fournisseurs=$bdd->prepare('SELECT * FROM clients WHERE id_entreprise=? ') ;
                            ...
                            <div class="form-group row">
                              ...
                              <div class="col-sm-3">
                              <input type="text" value='<?= $reponse['marchandise']?>' name='marchandise'class="form-control" id="inputCity" required placeholder="EX: Tomate">
                              </div>
                              <label for="inputPassword" class="col-sm-1 col-form-label">Quantité  </label>
                              <div class="col-sm-2">
                              <input type="number" value='<?= $reponse['quantite']?>'  name='quantite'class=" form-control" id="inputCity " required placeholder="EX: 10 pièces">
                              </div>
                              <label for="inputPassword" class="col-sm-1 col-form-label">Prix  :  </label>
                              <div class="col-sm-2">
                              <input type="number" value='<?= $reponse['prix']?>' name='prix'class="form-control" id="inputCity" required placeholder="Mettez le prix">
                              </div>
                            </div><hr>
                            <div class="form-group row">
                              <label for="inputPassword" class="col-sm-2 col-form-label">Réductions   :  </label>
                              <div class="col-sm-2">
                              ...
                              <div class="form-group row">
                              <label for="inputPassword" class="col-sm-2 col-form-label">Exompte  :  </label>
                              <div class="col-sm-2">
                            ...
                            <div class="form-group row">
                              <label for="inputPassword" class="col-sm-2 col-form-label">Transport  :  </label>
                              <div class="col-sm-2">
                              <input type="number"  name='montant_transport'class="form-control" id="inputCity "  placeholder="  Ex: 100 ">
                              </div>
                            </div><hr>
                            <div class="form-group row">
                              <label for="inputPassword" class="col-sm-2 col-form-label">TVA  :  </label>
                              <div class="col-sm-2">
                            ...
                            <div class="form-group row">
                              <label for="inputPassword" class="col-sm-2 col-form-label">Emballage  :  </label>
                              <div class="col-sm-2">
                              <input type="number"  name='montant_emballage'class="form-control" id="inputCity "  placeholder="  Ex: 100 ">
                              </div>
                            </div><hr>
                            <div class="form-group row">
                              <label for="inputPassword" class="col-sm-2 col-form-label">Avance  :  </label>
                              <div class="col-sm-2">
                              <input type="number"  name='montant_avance'class="form-control" id="inputCity "  placeholder="  Ex: 100 ">
                              </div>
                            </div><hr>
                            <div class="form-group row">
                              <label for="inputPassword" class="col-sm-3 col-form-label">Numéro Banque  :  </label>
                              <div class="col-sm-3">
                            ...
                            <?php
                              ...
                              <input type="number"  name='montant_payer'class="form-control" id="inputCity "  placeholder="  Ex: 100 ">
                                ...
                                <div class="form-group text-center">
                                    <input type="submit" class='bg-success text-white' value="Valider" name="valider"c>
                                </div>
                                </form>
                            </div>
                            </div>
