...
?>
<?php
 $req=$bdd->prepare('SELECT * FROM stoks_materiels WHERE id=?');
 $req->execute(array($getid));
 $reponse=$req->fetch();
...
?>
 <?php 
       ...
       if (isset($_POST['marchandise'])
        ...
        $ventes = $bdd->prepare('UPDATE stoks_materiels set quantiteS=quantiteS- ? where id= ? ');
        $ventes->execute(array($quantite, $_GET['id']));
      $insert=$bdd->prepare('INSERT INTO vente_materiels
      (
        id_client,
        id_entreprise,
        marchandise,
      ...
      (
        ...
        $montant_emballage,$montant_avance,$condition_payement,$montant_payer
        )
    );
    header('Location:index.php?page=ventes_materiels&id='.$getid);
...
<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
 ...
 Factures Ventes de materiels <span style='color:green'> ( <?=$vente_materiels?>)</span>
  ...
  <a  class="nav-link  " style="font-size:1.1rem; color:#A41FDE "  href="#">              
<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M14.5 3h-13a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
  <path fill-rule="evenodd" d="M5 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 5 8zm0-2.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zm0 5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5z"/>
  <circle cx="3.5" cy="5.5" r=".5"/>
 ...
 Etablir une facture de vente <?= $reponse['marchandiseS'] ?>
  </a>
     ...
     <div class="container">
        <div class="col-lg-8">
                            ...
                            <div class="card-body">
                               ...
                               Etablir une facture de vente de <?= $reponse['marchandiseS'] ?>
                                </h1><hr>
                            ...
                            <div class="form-group row">
                              ...
                              <div class="col-sm-3">
                              <input type="text" value='<?= $reponse['marchandiseS']?>' name='marchandise'class="form-control" id="inputCity" required placeholder="EX: Tomate">
                              </div>
                              <label for="inputPassword" class="col-sm-1 col-form-label">Quantité  </label>
                              <div class="col-sm-2">
                              <input type="number" value='<?= $reponse['quantiteS']?>'  name='quantite'class=" form-control" id="inputCity " required placeholder="EX: 10 pièces">
                              </div>
                            ...
                            <?php
                              ...
                              <div class="col-sm-3">
                              <input type="number" value='<?=$banque['montant']?>' name='montant_payer'class="form-control" id="inputCity "  placeholder="  Ex: 100 ">
                              </div>
                            </div><hr>
