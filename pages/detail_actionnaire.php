...
if (isset($_GET['id'])) {
  $getid=$_GET['id'];
 $req=$bdd->prepare('SELECT *from liberations where id_actionnaire=? order by id desc');
 $req->execute(array($getid));
 $comptes_libeation=$req->rowcount();
}?>
<?php
  $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
if (isset($_GET['id'])) {
  $getid=$_GET['id'];
 $req=$bdd->prepare("SELECT id, id_actionnaire,apport,numero_banque,montant_payer,DATE_FORMAT(date,'%d/%m/%Y à %Hh%imin%ss') AS date from liberations where id_actionnaire=? order by id desc");
 $req->execute(array($getid));
}?>
 
<?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $select=$bdd->prepare("SELECT id, id_entreprise ,nom,prenom, telephone,adresse, logo,montant_souscrit,DATE_FORMAT(date,'%d/%m/%Y à %Hh%imin%ss') AS date FROM actionnaires where id=? ORDER BY id DESC");
   ...
   <?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $nombre=$bdd->query('SELECT * FROM actionnaires ORDER BY id DESC');
   $count=$nombre->rowcount();
   $nombres=$nombre->fetch();
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $capitals=$bdd->query('SELECT * FROM capitals ORDER BY id DESC');
   $capital=$capitals->fetch();
   $compte=$nombre->rowcount();
   $part=$resultat['montant_souscrit']/$capital['capital']*100;
...
<?php
  ...
  <div class="row">
     ...
     <a  class="nav-link p-3  mb-1 shadow-sm bg-light bg-light  bg-light " style="font-size:1.3rem;" href="index.php?page=actionnaires&id=<?=$_SESSION['id'] ?>">
     <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-person-fill" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
     <path fill-rule="evenodd" d="M3 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1H3zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/>
     </svg>
      Actionnaire
     </a>   
     <a  class="nav-link  p-3  mb-1 shadow-sm bg-light bg-light  " style="font-size:1.2rem; color:#A41FDE"  href="index.php?page=detail_actionnaire&id=<?=$_GET['id']?>"  ?>
    <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-person-fill" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path fill-rule="evenodd" d="M3 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1H3zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/>
    </svg>
    Données de <?= $resultat['nom'] ?>
    </a>
              <?php if ($resultat['montant_souscrit']>0) {
              ?>
<a style="font-size:1.2rem; color:orange; font-family:arial" class="nav-link p-3  mb-1 shadow-sm bg-light  bg-light bg-light  " href="index.php?page=JournalActionnaires&id=<?=$_GET['id'] ?>"><svg width="1em" height="1.5em" viewBox="0 0 16 16" class="bi bi-book" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    ...
    <a style="font-size:1.2rem;" class="nav-link  p-3  mb-1 shadow-sm bg-light bg-light text-success" href="index.php?page=modifier_actionnaire&id=<?=$resultat['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" d="M13.498.795l.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
                </svg> Modifier Actionnaire </a>
                <a  style="font-size:1.2rem;" class="nav-link  p-3  mb-1 shadow-sm bg-light bg-light text-danger" href="index.php?page=supprimer_actionnaire&id=<?=$resultat['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-x" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
               <path fill-rule="evenodd" d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708z"/>
               </svg> Supprimer Actionnaire </a>
    ...
    <?php 
       if ($resultat['montant_souscrit']>0) {
    ...
    <div class="col-lg-12 mt-3 ">
        ...
        <a class="nav-link text-light " style="font-size:1.2rem;"   href="index.php?page=liberation_action&id=<?=$resultat['id'] ?>">
          <div class="row">
          <div class="col-3"> </div>
          <div class="col-lg-6 text-center">
           <?="<img src='".$resultat['logo']."' class='card-img' ' alt=''>" ;?><br>
           <h1>Liberer une action</h1>
           </div>
    ...
    <div class="col-12 text-white text-center p-2 bg-info" >
  <h4>  Souscriptions</h4>
    </div>
     <div class="col-6"style='border:solid gray 1px' >
      <h5> Date de souscription</h5>
     </div>
     <div class="col-6" style='border:solid gray 1px'>
       <h5>Montant Souscript non Libérer</h5>
     </div>
      <div class="col-6 text-right p-2"style='border:solid gray 1px'  >
   Le <?= $resultat['date'] ?>
    </div>
     <div class="col-6 text-right p-2" style='border:solid gray 1px' >
   <?= $resultat['montant_souscrit'] ?> dh 
    </div>
</div>
</div>
<?php
    ...
    ?>
            ...
            <?php
            if ( $comptes_libeation>0) {
             ?>
<div class="row mt-3 mr-1 ml-1">
<div class="col-12 text-center  bg-secondary text-white"  style='border:solid gray 1px' >
   <h4>Libérations</h4>
   </div>
   <div class="col-4"  style='border:solid gray 1px'>
   <h5>  Date de liberation</h5>
   </div>
   <div class="col-4" style='border:solid gray 1px' >
    <h5> Le type d'Apport</h5>
   </div>
   <div class="col-4" style='border:solid gray 1px' >
 <h5>  Montant libérer</h5>
   </div>
             <?php 
            }
...
while ($reponse=$req->fetch()) {
  ...
  ?>
  <div class="col-12">
  <a  href="index.php?page=detail_action&id=<?=$reponse['id'] ?>" style='color: black ; text-decoration:none'>
    <div class="row">
    
   <div class="col-4 text-right "  style='border:solid gray 1px' >
     Le <?= $reponse['date'] ?>
   </div>
   <div class="col-4 text-right "  style='border:solid gray 1px' >
      <?= $reponse['apport'] ?> 
