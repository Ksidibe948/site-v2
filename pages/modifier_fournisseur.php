           ...
           if ( isset($_POST['nom'])
             ...
             AND isset($_POST['adresse'])
          ...
          
                                {
                                                       ...
                                                       $req=$bdd->prepare('UPDATE  fournisseurs SET nom=? WHERE id=?');
                                                       $req->execute(array($nom,$_GET['id']));
                                                       header('Location:index.php?page=fournisseurs&id='.$donnees['id']);            
                                                       $req=$bdd->prepare('UPDATE  fournisseurs SET telephone=? WHERE id=?');
                                                       $req->execute(array($telephone,$_GET['id']));
                                                       header('Location:index.php?page=fournisseurs&id='.$donnees['id']);

                                                       $req=$bdd->prepare('UPDATE  fournisseurs SET adresse=? WHERE id=?');
                                 ...
                                 if (isset($_FILES['monfichier']) AND $_FILES['monfichier']['error'] == 0)
                                {
                                        ...
                                        if ($_FILES['monfichier']['size'] <= 1000000)
                                        {
                                                ...
                                                if (in_array($extension_upload, $extensions_autorisees))
                                                {
                                                        ...
                                                        $req=$bdd->prepare('UPDATE  fournisseurs SET logo=? WHERE id=?');
...
if (isset($_GET['id'])) {
  $getid=$_GET['id'];
   ...
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $select=$bdd->prepare('SELECT * FROM fournisseurs where id=? ORDER BY id DESC');
...
$bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
  ...
  <a style="font-size:1.2rem;color:#A41FDE" class="nav-link  " href="#"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
               <path fill-rule="evenodd" d="M13.498.795l.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
               </svg> Modification du compte <?=  $donnees['nom'] ?></a>
          <a class="nav-link  " style="font-size:1.2rem; "  href="index.php?page=nouveau_fournisse&id=<?=$_SESSION['id'] ?>">
        ...
        <div class="col-lg-8">
                            ...
                            <div class="card-body">
                              ...
                              <label for="inputZip">Nom</label>
                                <input type="text" value='<?=  $donnees['nom'] ?>' name='nom'class="form-control mr-5" id="inputCity"  placeholder="GOOGLE">
                              </div>
                              </div>
                              ...
                              <label for="inputZip">téléphone</label>
                                <input type="tel" value='<?=  $donnees['telephone'] ?>'   name='telephone'class="form-control mr-5" id="inputCity"  placeholder="EX: +212 6 40 63 71 57">
                              </div>
                              </div>  
                              ...
                              <label for="inputZip">Adresse</label>
                                <input type="text" value='<?=  $donnees['adresse'] ?>'     name='adresse'class="form-control mr-5" id="inputCity"  placeholder="Paris5, rue=2, porte=20">
                              </div>
                              </div> 
                              ...
                              <label for="inputZip">Logo client</label>
                                <input type="file" value='<?=  $donnees['logo'] ?>'     name='monfichier'class="form-control mr-5" id="inputCity" >
                              </div>
                              </div> 
