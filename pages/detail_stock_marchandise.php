...
<?php
$bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $listeachat_marchandise=$bdd->prepare('SELECT id, marchandiseS, quantiteS,  DATE_FORMAT(date,"%d/%m/%Y à %Hh%imin%ss") AS date FROM stoks_marchandise where id=? ORDER BY id DESC');
   $listeachat_marchandise->execute(array($_GET['id']));
   $verificaiton= $listeachat_marchandise->rowcount();
   $resultat=$listeachat_marchandise->fetch();
   ?>
 ...
 <?php
   ...
   $achats=$bdd->prepare("SELECT
       ...
       fournisseurs.logo,
       DATE_FORMAT(dates,'%d/%m/%Y à %Hh%imin%ss') AS date
