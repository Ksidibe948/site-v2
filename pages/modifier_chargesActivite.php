...
<?php
       ...
       {   
             ...
             $montant_th= $_POST['montant_th'];
             $taux_tva= $_POST["taux_tva"];
             $numero_banque= $_POST["numero_banque"];
            ...
            $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
            $modifications_bq = $resultat['montant_payement'] + $montant_payer; 
            ...
            $req=$bdd->prepare('UPDATE    chargeACtive SET nom_operation=? WHERE id=?');
            $req->execute(array($nom_operation,$getid));
            header('Location:visteurs.php?page= detail_chargesActivation&id='.$getid);
            $req=$bdd->prepare('UPDATE    chargeACtive SET montant_th=? WHERE id=?');
            $req->execute(array($montant_th,$getid));
            header('Location:visteurs.php?page= detail_chargesActivation&id='.$getid);
            $req=$bdd->prepare('UPDATE    chargeACtive SET taux_tva=? WHERE id=?');
            $req->execute(array($taux_tva,$getid));
            header('Location:visteurs.php?page= detail_chargesActivation&id='.$getid);
            $req=$bdd->prepare('UPDATE    chargeACtive SET numero_banque=? WHERE id=?');
            $req->execute(array($numero_banque,$getid));
            header('Location:visteurs.php?page=detail_chargesActivation&id='.$getid);
            $req=$bdd->prepare('UPDATE    chargeACtive SET montant_payer=? WHERE id=?');
            $req->execute(array($montant_payer,$getid));
            header('Location:visteurs.php?page=detail_chargesActivation&id='.$getid);
           
                        
          $req=$bdd->prepare('UPDATE  chargeACtive SET systeme=? WHERE id=?');
          $req->execute(array($systeme,$getid));
          header('Location:visteurs.php?page=detail_chargesActivation&id='.$_GET['id']);
                              
          $req=$bdd->prepare('UPDATE  chargeACtive SET duree=? WHERE id=?');
          $req->execute(array($duree,$getid));
          header('Location:visteurs.php?page=detail_chargesActivation&id='.$_GET['id']);

                            
          $req=$bdd->prepare('UPDATE  chargeACtive SET mois=? WHERE id=?');
       ...
       <ul class="navbar-nav ml-5 ">
         ...
         <li class="nav-item dropdown">
          ...
          <?= $_SESSION['nom']?>
           ...
           <a class="nav-link text-secondary  " href="visteurs.php?page=publication&id=<?=$_SESSION['id'] ?>">
           <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-plus-square" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
           <path fill-rule="evenodd" d="M14 1H2a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1zM2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2H2z"/>
           <path fill-rule="evenodd" d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z"/>
           </svg> Publier
           </a>
           <a class="nav-link text-secondary  " href="visteurs.php?page=profil&id=<?=$_SESSION['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-person" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
             <path fill-rule="evenodd" d="M10 5a2 2 0 1 1-4 0 2 2 0 0 1 4 0zM8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm6 5c0 1-1 1-1 1H3s-1 0-1-1 1-4 6-4 6 3 6 4zm-1-.004c-.001-.246-.154-.986-.832-1.664C11.516 10.68 10.289 10 8 10c-2.29 0-3.516.68-4.168 1.332-.678.678-.83 1.418-.832 1.664h10z"/>
             </svg> Profil
           </a>
           <a class="nav-link text-secondary " href="visteurs.php?page=modifier_utilisateurs&id=<?=$_SESSION['id'] ?>">
           <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-person-plus" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
             <path fill-rule="evenodd" d="M8 5a2 2 0 1 1-4 0 2 2 0 0 1 4 0zM6 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm6 5c0 1-1 1-1 1H1s-1 0-1-1 1-4 6-4 6 3 6 4zm-1-.004c-.001-.246-.154-.986-.832-1.664C9.516 10.68 8.289 10 6 10c-2.29 0-3.516.68-4.168 1.332-.678.678-.83 1.418-.832 1.664h10zM13.5 5a.5.5 0 0 1 .5.5V7h1.5a.5.5 0 0 1 0 1H14v1.5a.5.5 0 0 1-1 0V8h-1.5a.5.5 0 0 1 0-1H13V5.5a.5.5 0 0 1 .5-.5z"/>
             </svg> Modifier  profil
           </a>
           <a class="nav-link text-secondary " href="visteurs.php?page=deconnexion"> 
...
<div class="row">
     ...
     <div class="overflow-auto" style='width:72%;
       height:800px;'>
        <ul  class="navbar-nav  mr-auto ">
         <li class="nav-item ">
         <a   class="nav-link p-3  mb-1 shadow-sm bg-light text-secondary   " style='  font-size:1.3rem'  href="visteurs.php?page=utilisateurs">
        ...
        <path fill-rule="evenodd" d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8zm7.5-6.923c-.67.204-1.335.82-1.887 1.855A7.97 7.97 0 0 0 5.145 4H7.5V1.077zM4.09 4H2.255a7.025 7.025 0 0 1 3.072-2.472 6.7 6.7 0 0 0-.597.933c-.247.464-.462.98-.64 1.539zm-.582 3.5h-2.49c.062-.89.291-1.733.656-2.5H3.82a13.652 13.652 0 0 0-.312 2.5zM4.847 5H7.5v2.5H4.51A12.5 12.5 0 0 1 4.846 5zM8.5 5v2.5h2.99a12.495 12.495 0 0 0-.337-2.5H8.5zM4.51 8.5H7.5V11H4.847a12.5 12.5 0 0 1-.338-2.5zm3.99 0V11h2.653c.187-.765.306-1.608.338-2.5H8.5zM5.145 12H7.5v2.923c-.67-.204-1.335-.82-1.887-1.855A7.97 7.97 0 0 1 5.145 12zm.182 2.472a6.696 6.696 0 0 1-.597-.933A9.268 9.268 0 0 1 4.09 12H2.255a7.024 7.024 0 0 0 3.072 2.472zM3.82 11H1.674a6.958 6.958 0 0 1-.656-2.5h2.49c.03.877.138 1.718.312 2.5zm6.853 3.472A7.024 7.024 0 0 0 13.745 12H11.91a9.27 9.27 0 0 1-.64 1.539 6.688 6.688 0 0 1-.597.933zM8.5 12h2.355a7.967 7.967 0 0 1-.468 1.068c-.552 1.035-1.218 1.65-1.887 1.855V12zm3.68-1h2.146c.365-.767.594-1.61.656-2.5h-2.49a13.65 13.65 0 0 1-.312 2.5zm2.802-3.5h-2.49A13.65 13.65 0 0 0 12.18 5h2.146c.365.767.594 1.61.656 2.5zM11.27 2.461c.247.464.462.98.64 1.539h1.835a7.024 7.024 0 0 0-3.072-2.472c.218.284.418.598.597.933zM10.855 4H8.5V1.077c.67.204 1.335.82 1.887 1.855.173.324.33.682.468 1.068z"/>
         ...
         <a  class="nav-link  p-3  mb-1 shadow-sm bg-light text-secondary  " style="font-size:1.3rem"  href="visteurs.php?page=comptabilite&id=<?=$_SESSION['id'] ?>">
        ...
        <a  class="nav-link  p-3  mb-1 shadow-sm bg-light text-secondary " style="font-size:1.3rem;  " href="visteurs.php?page=comptabilité_generale&id=<?=$_SESSION['id'] ?>">
      ...
      <a  class="nav-link p-3  mb-1 shadow-sm bg-light text-secondary "  style="font-size:1.3rem;" href="visteurs.php?page=charges&id=<?=$_SESSION['id'] ?>"> 
  ...
  <a  class="nav-link mb-1 shadow-sm bg-light text-secondary  "  style="font-size:1.2rem;" href="visteurs.php?page=ChargesActive&id=<?=$_SESSION['id'] ?>"> 
  <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-fullscreen-exit" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M5.5 0a.5.5 0 0 1 .5.5v4A1.5 1.5 0 0 1 4.5 6h-4a.5.5 0 0 1 0-1h4a.5.5 0 0 0 .5-.5v-4a.5.5 0 0 1 .5-.5zm5 0a.5.5 0 0 1 .5.5v4a.5.5 0 0 0 .5.5h4a.5.5 0 0 1 0 1h-4A1.5 1.5 0 0 1 10 4.5v-4a.5.5 0 0 1 .5-.5zM0 10.5a.5.5 0 0 1 .5-.5h4A1.5 1.5 0 0 1 6 11.5v4a.5.5 0 0 1-1 0v-4a.5.5 0 0 0-.5-.5h-4a.5.5 0 0 1-.5-.5zm10 1a1.5 1.5 0 0 1 1.5-1.5h4a.5.5 0 0 1 0 1h-4a.5.5 0 0 0-.5.5v4a.5.5 0 0 1-1 0v-4z"/>
  </svg>
 Charges Activés
        </a>
        <a style="font-size:1.2rem;" class="nav-link  text-success mb-1 shadow-sm bg-light" href="visteurs.php?page=modifier_chargesActivite&id=<?=$resultat['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M13.498.795l.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
  </svg> Modifier  <?=$resultat['nom_operation'] ?> </a>
    
    <a style="font-size:1.3rem; " class="nav-link p-3  mb-1 shadow-sm bg-light text-secondary " href="visteurs.php?page=banque&id=<?=$_SESSION['id'] ?>">
   ...
   <a   class="nav-link p-3  mb-1 shadow-sm bg-light text-secondary " style="font-size:1.3rem;" href="visteurs.php?page=capital&id=<?=$_SESSION['id'] ?>">
   ...
   <a  class="nav-link p-3  mb-1 shadow-sm bg-light text-secondary " style="font-size:1.3rem;" href="visteurs.php?page=actionnaires&id=<?=$_SESSION['id'] ?>">
   ...
   <a class="nav-link p-3  mb-1 shadow-sm bg-light text-secondary " style="font-size:1.3rem; "   href="visteurs.php?page=clients&id=<?=$_SESSION['id'] ?>">              
   ...
   <a  class="nav-link p-3  mb-1 shadow-sm bg-light text-secondary" style="font-size:1.3rem; " href="visteurs.php?page=fournisseurs&id=<?=$_SESSION['id'] ?>">              
   ...
   <a  class="nav-link p-3  mb-1 shadow-sm bg-light text-secondary " style="font-size:1.3rem; "  href="visteurs.php?page=factures&id=<?=$_SESSION['id'] ?>">              
   <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    ...
    Factures
     ...
     <a  class="nav-link p-3  mb-1 shadow-sm bg-light text-secondary "  style="font-size:1.3rem;" href="visteurs.php?page=produits&id=<?=$_SESSION['id'] ?>"> 
    ...
    Produits
      ...
      <a  class="nav-link p-3  mb-1 shadow-sm bg-light text-secondary " style="font-size:1.3rem;" href="visteurs.php?page=Stocks&id=<?=$_SESSION['id'] ?>"> 
     <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-cart3" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
     <path fill-rule="evenodd" d="M0 1.5A.5.5 0 0 1 .5 1H2a.5.5 0 0 1 .485.379L2.89 3H14.5a.5.5 0 0 1 .49.598l-1 5a.5.5 0 0 1-.465.401l-9.397.472L4.415 11H13a.5.5 0 0 1 0 1H4a.5.5 0 0 1-.491-.408L2.01 3.607 1.61 2H.5a.5.5 0 0 1-.5-.5zM3.102 4l.84 4.479 9.144-.459L13.89 4H3.102zM5 12a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm7 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm-7 1a1 1 0 1 0 0 2 1 1 0 0 0 0-2zm7 0a1 1 0 1 0 0 2 1 1 0 0 0 0-2z"/>
    </svg>
    Stocks
   </a>
   <a  class="nav-link p-3  mb-1 shadow-sm bg-light text-secondary "style="font-size:1.3rem;"  href="visteurs.php?page=balance&id=<?=$_SESSION['id'] ?>">
   ...
   <a  class="nav-link p-3  mb-1 shadow-sm bg-light text-secondary" style="font-size:1.3rem;" href="visteurs.php?page=Bilan&id=<?=$_SESSION['id'] ?>">
    ...
    <a  class="nav-link p-3  mb-1 shadow-sm bg-light text-secondary  " style="font-size:1.3rem;" href="visteurs.php?page=cpc&id=<?=$_SESSION['id'] ?>"> 
         ...
         <div class="row">
              ...
              <!-- la partie inscription -->
                            ...
                            <div class="card-body">
                                ...
                                Modifier <?=$resultat['nom_operation'] ?>
                                </h1><hr>
                                <form action="" method="POST" enctype="multipart/form-data">
                                
                              <div class="form-group row">
                                    <div class="col-lg-2"></div>
                              <label for="inputPassword" class="col-lg-3 col-form-label "><h5>Nom de l'operation : </h5></label>
                              <div class="col-lg-5">
                            ...
                            <div class="form-group row">
                          <div class="col-lg-2"></div>
                              <label for="inputPassword" class="col-lg-3 col-form-label "><h5>Montant_th  HT  : </h5></label>
                              <div class="col-lg-5">
                            ...
                            <div class="form-group row">
                          <div class="col-lg-2"></div>
                              <label for="inputPassword" class="col-lg-3 col-form-label "><h5>TVA   : </h5></label>
                               <div class="col-lg-5">
                            ...
                            <div class="form-group row">
                          <div class="col-lg-2"></div>
                              <label for="inputPassword" class="col-lg-3 col-form-label "><h5> Bancaire  : </h5></label>
                               <div class="col-lg-5">
                            ...
                            <div class="form-group row">
                          <div class="col-lg-2"></div>
                              <label for="inputPassword" class="col-lg-3 col-form-label "><h5>Montant payer : </h5></label>
                               <div class="col-lg-5">
...
<div class="form-group row">
    <div class="col-lg-2"></div>
<label for="inputPassword" class="col-sm-3  col-form-label "><h5>Mois de mis  en service : </h5></label>
 <div class="col-lg-5">
 <select id="inputState" name="mois" value='<?= $resultat['mois'] ?>' class="form-control mr-5">
   <option>1</option>
   <option>2</option>
   <option>3</option>
   <option>4</option>
 ...
 <div class="form-group row">
       <div class="col-lg-2"></div>
<label for="inputPassword" class="col-sm-3  col-form-label"> <h5>Système  : </h5></label>
<div class="col-lg-5">
<select id="inputState" name="systeme" value='<?= $resultat['systeme'] ?>' class="form-control mr-5">
 ...
 <div class="form-group row">
     <div class="col-lg-2"></div>
<label for="inputPassword" class="col-sm-3  col-form-label"><h5>Durées de vie  : </h5></label>
<div class="col-lg-5">
<input type="number"  name='duree' value='<?= $resultat['duree'] ?>' class=" form-control" id="inputCity " required placeholder="EX: 5 ans">
</div>
</div>
