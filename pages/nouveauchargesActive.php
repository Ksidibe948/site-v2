...
<?php
       ...
       {   
            ...
            $req=$bdd->prepare('UPDATE    produits_exploitation SET montant_th=? WHERE id=?');
            $req->execute(array($montant_th,$_SESSION['id']));

            $modification = $bdd->prepare("update banque set montant = montant -  ? where id = ? ");
            $modification->execute(array($montant_payer,$numero_banque,$systeme,$duree,$mois ));
        
             $insert=$bdd->prepare("INSERT INTO 
            chargeACtive(id_entreprise,nom_operation,montant_th, taux_tva,numero_banque,montant_payer,
        systeme,duree,mois)
             VALUES(?,?,?,?,?,? ,?,?,?)
              ");
             $insert->execute(array( $_SESSION['id'],$nom_operation,  $montant_th,$taux_tva, $numero_banque, $montant_payer,$systeme,$duree,$mois));
            
             header('Location:visteurs.php?page=ChargesActive&id='.$_SESSION['id']);
      }                 
         ?>
...
<div class="row">
 ...
 <a style='font-size:1.2rem' class="nav-link text-info mb-1 shadow-sm bg-light   "  href="visteurs.php?page=nouveauchargesActive&id=<?=$_SESSION['id'] ?>">
             <svg width="1.1em" height="1.1em" viewBox="0 0 16 16" class="bi bi-plus" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
             <path fill-rule="evenodd" d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z"/>
             </svg>
  Activé une nouvelle charge
 </a>
   ...
   <div class="col-lg-6 col-12"> 
              ...
              <div class="col-lg-12">
                            ...
                            <div class="card-body">
                                <h1 class="card-title p-4 bg-secondary text-light p-2 text-center">
                                Activé une Nouvelle charge
                                </h1><hr>
                              ...
                              <div class="col-lg-5">
                              <input type="text"   name='nom_operation'class="form-control" id="inputCity "  placeholder="  Ex: gain de change ">
                              </div>
                            </div>
                            <div class="form-group row">
                          <div class="col-lg-2"></div>
                              ...
                              <div class="col-lg-5">
                              <input type="number"   name='montant_th'class="form-control" id="inputCity "  placeholder="  Ex: 1000 ">
                              </div>
                            </div>
