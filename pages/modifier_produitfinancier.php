...
if (isset($_GET['id'])) {
   ...
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $select=$bdd->prepare('SELECT * FROM    produits_financier WHERE id=? ORDER BY id DESC');
...
<?php
       ...
       {   
            ...
            $req=$bdd->prepare('UPDATE    produits_financier SET nom_operation=? WHERE id=?');
            $req->execute(array($nom_operation,$getid));
            header('Location:index.php?page= produits_financier&id='.$getid);
            $req=$bdd->prepare('UPDATE    produits_financier SET montant_th=? WHERE id=?');
            $req->execute(array($montant_th,$getid));
            header('Location:index.php?page= produits_financier&id='.$getid);
            $req=$bdd->prepare('UPDATE    produits_financier SET taux_tva=? WHERE id=?');
            $req->execute(array($taux_tva,$getid));
            header('Location:index.php?page= produits_financier&id='.$getid);
            $req=$bdd->prepare('UPDATE    produits_financier SET numero_banque=? WHERE id=?');
            $req->execute(array($numero_banque,$getid));
            header('Location:index.php?page=detail_produitfinancier&id='.$getid);
            $req=$bdd->prepare('UPDATE    produits_financier SET montant_payer=? WHERE id=?');
...
$bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
  ...
  <?php
   ...
   <?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $detail_produitfinancier=$bdd->prepare('SELECT * FROM  produits_financier where id_entreprise=? ORDER BY id DESC');
   $detail_produitfinancier->execute(array($_SESSION['id']));
   $detail_produitfinancier=$detail_produitfinancier->rowcount();
   ?>
<?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
 ...
 <a style="font-size:1.2rem;" class="nav-link  text-success p-3  mb-1 shadow-sm bg-light  bg-light bg-light " href="index.php?page=modifier_produitfinancier&id=<?=$resultat['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- la partie inscription -->
                            ...
                            <div class="card-body">
                                ...
                                Enregistre un Nouveau Produit Financière
                                </h1>
