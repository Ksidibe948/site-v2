...
?>
 ...
 <?php 
       ...
       if (isset($_POST['marchandise'])
         ...
         $modifications = $bdd->prepare("update achats set net_a_payer = net_a_payer -  ? where id = ? ");
         $modifications->execute(array($montant_payer,$_GET['id']));
 

      $insert=$bdd->prepare('INSERT INTO PayementMarchandise
      (
        id_client,
        id_entreprise,
        marchandise,
...
<?php
$bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $achats_marchandise=$bdd->prepare('SELECT * FROM achats where id=? ORDER BY id DESC');
