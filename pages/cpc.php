...
<?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $select_produitsexploitation=$bdd->prepare('SELECT id,id_entreprise,nom_operation,montant_th, 
     DATE_FORMAT(date,"%d/%m/%Y à %Hh%imin%ss") AS date
    FROM  produits_exploitation where id_entreprise=?  ORDER BY id DESC');
  $select_produitsexploitation->execute(array($_SESSION['id']));
   ?>


<?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $select_produitsfinancier=$bdd->prepare('SELECT id,id_entreprise,nom_operation,montant_th, 
     DATE_FORMAT(date,"%d/%m/%Y à %Hh%imin%ss") AS date
    FROM  produits_financier where id_entreprise=?  ORDER BY id DESC');
   $select_produitsfinancier->execute(array($_SESSION['id']));

   ?>
   <?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $select_produitsexceptionnel=$bdd->prepare('SELECT id,id_entreprise,nom_operation,montant_th, 
     DATE_FORMAT(date,"%d/%m/%Y à %Hh%imin%ss") AS date
    FROM  produits_execeptionnel where id_entreprise=?  ORDER BY id DESC');
   $select_produitsexceptionnel->execute(array($_SESSION['id']));
   ?>
   <?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $select_charges_exploitation=$bdd->prepare('SELECT id,id_entreprise,nom_operation,montant_th, 
     DATE_FORMAT(date,"%d/%m/%Y à %Hh%imin%ss") AS date
    FROM  charges_exploitation where id_entreprise=?  ORDER BY id DESC');
  $select_charges_exploitation->execute(array($_SESSION['id']));
   ?>
   <?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $select_charges_financieres=$bdd->prepare('SELECT id,id_entreprise,nom_operation,montant_th, 
     DATE_FORMAT(date,"%d/%m/%Y à %Hh%imin%ss") AS date
    FROM  charges_financieres where id_entreprise=?  ORDER BY id DESC');
   $select_charges_financieres->execute(array($_SESSION['id']));
   ?>
   <?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $select_chargenoncourants=$bdd->prepare('SELECT id,id_entreprise,nom_operation,montant_th, 
     DATE_FORMAT(date,"%d/%m/%Y à %Hh%imin%ss") AS date
    FROM  charges_noncourantes where id_entreprise=?  ORDER BY id DESC');
   $select_chargenoncourants->execute(array($_SESSION['id']));
   ?>
<?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
...
<div class="row">
  ...
  <div class="overflow-auto" style='width:70%;
    height:1000px;'>
     <ul  class="navbar-nav pl-3 mr-auto mt-5mb-5 mt-lg-0">
      <li class="nav-item ">
  ...
  <hr>
  <a  class="nav-link pl-2 " style="font-size:1.2rem; color:#A41FDE" href="index.php?page=cpc&id=<?=$_SESSION['id'] ?>"> 
 ...
 <a style="font-size:1.2rem; " class="nav-link pl-2 " href="index.php?page=banque&id=<?=$_SESSION['id'] ?>">
...
<a   class="nav-link pl-2 " style="font-size:1.2rem;" href="index.php?page=capital&id=<?=$_SESSION['id'] ?>">
...
<a  class="nav-link pl-2 " style="font-size:1.2rem;" href="index.php?page=actionnaires&id=<?=$_SESSION['id'] ?>">
...
<a class="nav-link pl-2 " style="font-size:1.2rem; "   href="index.php?page=clients&id=<?=$_SESSION['id'] ?>">              
...
<a  class="nav-link pl-2 " style="font-size:1.3rem; "  href="index.php?page=factures&id=<?=$_SESSION['id'] ?>">              
<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
 ...
 Factures
  ...
  <a  class="nav-link pl-2 "  style="font-size:1.2rem;" href="index.php?page=produits&id=<?=$_SESSION['id'] ?>"> 
 ...
 <a  class="nav-link pl-2 "  style="font-size:1.2rem;" href="index.php?page=charges&id=<?=$_SESSION['id'] ?>"> 
 ...
 Charges
   ...
   <a  class="nav-link pl-2 " style="font-size:1.2rem;" href="index.php?page=Stocks&id=<?=$_SESSION['id'] ?>"> 
  <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-cart3" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M0 1.5A.5.5 0 0 1 .5 1H2a.5.5 0 0 1 .485.379L2.89 3H14.5a.5.5 0 0 1 .49.598l-1 5a.5.5 0 0 1-.465.401l-9.397.472L4.415 11H13a.5.5 0 0 1 0 1H4a.5.5 0 0 1-.491-.408L2.01 3.607 1.61 2H.5a.5.5 0 0 1-.5-.5zM3.102 4l.84 4.479 9.144-.459L13.89 4H3.102zM5 12a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm7 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm-7 1a1 1 0 1 0 0 2 1 1 0 0 0 0-2zm7 0a1 1 0 1 0 0 2 1 1 0 0 0 0-2z"/>
 </svg>
 Stocks
</a><hr>
<a  class="nav-link pl-2 "style="font-size:1.2rem;"  href="index.php?page=balance&id=<?=$_SESSION['id'] ?>">
...
<div class="col-lg-8 col-12  mt-5  ">    
  ...
  <div class="card shadow-lg pl-4 pb-5 pt-5 pr-4 w-100">
   <div class="row ">
           <div class="col-12 text-center text-light bg-secondary">
            <h3 style='padding-top:10px'>Charges et Produits de <?=$_SESSION['nom'] ?></h3>  
           </div>
         </div>
         <div class="row ">
              <div class="col-8 pt-2 text-center bg-light text-dark"style='border:solid 0.5px gray'>
           <h5> Elements</h5> 
              </div>
              <div class="col-4 pt-2 text-center bg-light text-dark"style='border:solid 0.5px gray'>
            <h5>Montants</h5>
              </div>
              </div>
          <div class="row ">
              <div class="col-8 pt-2 text-left bg-light text-dark"style='border:solid 0.5px gray'>
           <h6>Produits d'exploitation</h6>
              </div> 
              <div class="col-4 pt-2 text-right bg-light text-dark"style='border:solid 0.5px gray'>
           .......
              </div>
              </div>
              <?php
    while($resultat= $select_produitsexploitation->fetch())
    {
        ?>
        <a class="link text-dark " href="index.php?page=detail_produitdexploitation&id=<?=$resultat['id'] ?>" style='text-decoration:none'>
          <div class="row  ">
   ...
   <?php
    while($resultat= $select_charges_exploitation->fetch())
    {
        ?>
        <a class="link text-dark " href="index.php?page=detail_chargesdexploitation&id=<?=$resultat['id'] ?>" style='text-decoration:none'>
          <div class="row  ">
          <div class="col-8 pt-2 text-left"style='border:solid 0.5px gray'>
          <?= $resultat['nom_operation'] ?>
          </div>
    ...
    ?>
          ...
          <h6>  Produits Financiers</h6>
           ...
           .......
              ...
              <?php
    while($resultat= $select_produitsfinancier->fetch())
    {
        ?>
        <a class="link text-dark " href="index.php?page=detail_produitfinancier&id=<?=$resultat['id'] ?>" style='text-decoration:none'>
          <div class="row  ">
          <div class="col-8 pt-2 text-left"style='border:solid 0.5px gray'>
          <?= $resultat['nom_operation'] ?>
          </div>
    ...
    ?>
           ...
           .......
              ...
              <?php
    while($resultat= $select_charges_financieres->fetch())
    {
        ?>
        <a class="link text-dark " href="index.php?page=detail_chargesfinanciere&id=<?=$resultat['id'] ?>" style='text-decoration:none'>
          <div class="row  ">
          <div class="col-8 pt-2 text-left"style='border:solid 0.5px gray'>
          <?= $resultat['nom_operation'] ?>
          </div>
    ...
    <div class="row ">
         ...
         <h6>  Produits Non Courants</h6>
           ...
           .......
              ...
              <?php
    while($resultat= $select_produitsexceptionnel->fetch())
    {
        ?>
        <a class="link text-dark " href="index.php?page=detail_produits_exceptionnel&id=<?=$resultat['id'] ?>" style='text-decoration:none'>
          <div class="row  ">
          <div class="col-8 pt-2 text-left"style='border:solid 0.5px gray'>
          <?= $resultat['nom_operation'] ?>
          </div>
    ...
    <?php
    while(   $select_chargenoncourants->fetch())
    {
        ?>
        <a class="link text-dark " href="index.php?page=detail_chargesexceptionnelles&id=<?=$resultat['id'] ?>" style='text-decoration:none'>
          <div class="row  ">
          <div class="col-8 pt-2 text-left"style='border:solid 0.5px gray'>
          <?= $resultat['nom_operation'] ?>
          </div>
