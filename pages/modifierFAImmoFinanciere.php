...
<?php
 if (isset($_GET['id'])) 
 {
  ...
  $select=$bdd->prepare('SELECT* FROM AchatsImmoFinanciere WHERE id=?');
...
?>
 ...
 <?php 
       ...
       ) 
       {
        ...
        $reqS=$bdd->prepare('UPDATE stocksimmofinanciere SET marchandiseS=? WHERE id=?');
        $reqS->execute(array($marchandise,$getid));
        header('Location:visteurs.php?page=DetailAchatImmoFinanciere&id='.$_GET['id']);
        
        $reqS=$bdd->prepare('UPDATE stocksimmofinanciere SET id_fournisseurS=? WHERE id=?');
        $reqS->execute(array($numero_client,$getid));
        header('Location:visteurs.php?page=DetailAchatImmoFinanciere&id='.$_GET['id']);
        
        $reqS=$bdd->prepare('UPDATE stocksimmofinanciere SET quantiteS=? WHERE id=?');
        $reqS->execute(array($quantite,$getid));
        header('Location:visteurs.php?page=DetailAchatImmoFinanciere&id='.$_GET['id']);
        
        $reqS=$bdd->prepare('UPDATE stocksimmofinanciere SET prixS=? WHERE id=?');
        ...
        $reqS=$bdd->prepare('UPDATE stocksimmofinanciere SET taux_tvaS=? WHERE id=?');
        $reqS->execute(array($taux_tva,$getid));
        header('Location:visteurs.php?page=DetailAchatImmoFinanciere&id='.$_GET['id']);
            

        $reqS=$bdd->prepare('UPDATE stocksimmofinanciere SET monyen_payementS=? WHERE id=?');
        $reqS->execute(array($condition_payement,$getid));
        header('Location:visteurs.php?page=DetailAchatImmoFinanciere&id='.$_GET['id']);
             
        $reqS=$bdd->prepare('UPDATE stocksimmofinanciere SET montant_payementS=? WHERE id=?');
        $reqS->execute(array($montant_payer,$getid));
        header('Location:visteurs.php?page=DetailAchatImmoFinanciere&id='.$_GET['id']);
        
        // MODIFICATIONIMMOBILISATIONFINANCIERE




         
          $req=$bdd->prepare('UPDATE AchatsImmoFinanciere SET marchandise=? WHERE id=?');
          $req->execute(array($marchandise,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoFinanciere&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE AchatsImmoFinanciere SET id_fournisseur=? WHERE id=?');
          $req->execute(array($numero_client,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoFinanciere&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE AchatsImmoFinanciere SET quantite=? WHERE id=?');
          $req->execute(array($quantite,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoFinanciere&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE AchatsImmoFinanciere SET prix=? WHERE id=?');
          ...
          $req=$bdd->prepare('UPDATE AchatsImmoFinanciere SET taux_tva=? WHERE id=?');
          $req->execute(array($taux_tva,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoFinanciere&id='.$_GET['id']);
              

          $req=$bdd->prepare('UPDATE AchatsImmoFinanciere SET monyen_payement=? WHERE id=?');
          $req->execute(array($condition_payement,$getid));
          header('Location:visteurs.php?page=DetailAchatImmoFinanciere&id='.$_GET['id']);
               
          $req=$bdd->prepare('UPDATE AchatsImmoFinanciere SET montant_payement=? WHERE id=?');
...
<?php
  ...
  <div class="row">
     ...
     <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
      ...
      Factures
       ...
       <a  class="nav-link p-3  mb-1 shadow-sm bg-light "  style="font-size:1.2rem;" href="visteurs.php?page=achat_immobilisation&id=<?=$_SESSION['id'] ?>"> 
       <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
       <path fill-rule="evenodd" d="M14.5 3h-13a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
       <path fill-rule="evenodd" d="M5 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 5 8zm0-2.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zm0 5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5z"/>
       <circle cx="3.5" cy="5.5" r=".5"/>
  ...
  <a  class="nav-link p-3  mb-1 shadow-sm bg-light"  style="font-size:1.2rem;color:#A41FDE" href="visteurs.php?page=AchatsImmobilisationsFinancières&id=<?=$_SESSION['id'] ?>"> 
  <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
   <path fill-rule="evenodd" d="M14.5 3h-13a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
   <path fill-rule="evenodd" d="M5 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 5 8zm0-2.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zm0 5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5z"/>
   <circle cx="3.5" cy="5.5" r=".5"/>
 ...
 <a style="font-size:1.2rem;" class="nav-link p-3  mb-1 shadow-sm bg-light text-success" href="visteurs.php?page=modifierFAImmoFinanciere&id=<?=$_GET['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  ...
  <div class="col-12">
                            ...
                            <div class="card-body">
                               ...
                               Modification de <?=$donnees['marchandise'] ?>
                              </h1><hr>
                                <form action="" method="POST" enctype="multipart/form-data">
                              <div class="form-group row">
                              <label for="inputPassword" class="col-sm-2 col-form-label"><h6>Fournisseurs :</h6> </label>
                              <div class="col-sm-4">
                            ...
                            <div class="form-group row">
                              <label for="inputPassword" class="col-sm-2 col-form-label"><h6>Immobilisatios</h6></label>
                              <div class="col-sm-3">
                            ...
                            <div class="form-group row">
                            <label for="inputPassword" class="col-sm-2 col-form-label"><h6>Numéro Banque</h6></label>
                              <div class="col-sm-4">
