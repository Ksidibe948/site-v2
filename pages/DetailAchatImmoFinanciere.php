...
<?php
   ...
   $achamarchandises=$bdd->prepare("SELECT
        AchatsImmoFinanciere.id,
        AchatsImmoFinanciere.id_fournisseur,
        AchatsImmoFinanciere. id_entreprise,
        AchatsImmoFinanciere.marchandise,
        AchatsImmoFinanciere.quantite,
        AchatsImmoFinanciere.prix,
        AchatsImmoFinanciere.taux_tva,
        AchatsImmoFinanciere. monyen_payement,
        AchatsImmoFinanciere. montant_payement,
      fournisseurs.nom,
      fournisseurs.telephone,
      fournisseurs. adresse,
      fournisseurs.logo,
        DATE_FORMAT(  AchatsImmoFinanciere.date,'%d/%m/%Y à %Hh%imin%ss') AS date
   FROM AchatsImmoFinanciere,fournisseurs WHERE AchatsImmoFinanciere.id_fournisseur=fournisseurs.id AND AchatsImmoFinanciere. id=? ORDER BY id DESC");
...
$bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
  ...
  $banques->execute(array( $_SESSION['id']));?>
   ...
   <?php
 $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $stocks_marchandises=$bdd->prepare('SELECT * FROM AchatsImmoFinanciere where id_entreprise=? ORDER BY id DESC');
...
<?php
  ...
  <a style="font-size:1.2rem; color:orange; p-3  mb-1 shadow-sm bg-light font-family:arial" class="nav-link  " href="visteurs.php?page=JournalFactureAchatImmoFinanciere&id=<?=$_GET['id'] ?>"><svg width="1em" height="1.5em" viewBox="0 0 16 16" class="bi bi-book" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
 ...
 <a style="font-size:1.2rem;" class="nav-link p-3  mb-1 shadow-sm bg-light text-success" href="visteurs.php?page=modifierFAImmoFinanciere&id=<?=$_GET['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    ...
    <a  style="font-size:1.2rem;" class="nav-link p-3  mb-1 shadow-sm bg-light text-danger" href="visteurs.php?page=supprimeFAImmoFinanciere&id=<?=$_GET['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-x" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
   <path fill-rule="evenodd" d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708z"/>
   </svg> Supprimer la facture de <?=$donnees['marchandise'] ?> </a> <hr> 
