...
if (isset ($_GET['id']))
{
   $get=$_GET['id'];
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
    $utilisateurs=$bdd->prepare("SELECT publications.fichier, publications.id_publications, publications.id_utilisateurs, publications.contenu,utilisateurs.nom,utilisateurs.email, utilisateurs.images ,DATE_FORMAT(date,  '%d/%m/%Y à %Hh%imin%ss') AS date FROM publications, utilisateurs WHERE publications.id_utilisateurs=utilisateurs.id AND id_publications=?");
...
<div class="container">
 ...
 <div class="card w-100 bg-white p-3 shadow-lg ">
  <div class='imagess'>
  <?="<img src='".$info_utilisateurs['fichier']."' class='card-img-top'  pt-2 pb-2  mt-2'   alt=''>" ;?> 
  </div>
</div>
</div>
</div>
