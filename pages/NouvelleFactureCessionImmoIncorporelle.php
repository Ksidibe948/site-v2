...
?>
<?php
 $req=$bdd->prepare('SELECT * FROM stocksimmoincorporelle WHERE id=?');
 $req->execute(array($getid));
 $reponse=$req->fetch();
...
?>
 <?php 
       ...
       if (isset($_POST['marchandise'])
        ...
        $ventes = $bdd->prepare('UPDATE stocksimmoincorporelle set quantiteS=quantiteS - ? where id= ? ');
        $ventes->execute(array($quantite, $_GET['id']));
      
      $insert=$bdd->prepare('INSERT INTO CessionImmoIncorporelle
      (
        id_client,
        id_entreprise,
        marchandise,
      ...
      (
        ...
        $taux_tva,
        $condition_payement,$montant_payer
        )
    );
    header('Location:visteurs.php?page=CessionImmoIncorporelle&id='.$getid);
...
<div class="row">
 ...
 <a style='font-size:1.3rem' class="nav-link p-3  mb-1 shadow-sm bg-light text-info "  href="visteurs.php?page=ListeAchatImmoIncorporelle&id=<?=$_SESSION['id'] ?>">
  <svg width="1.1em" height="1.1em" viewBox="0 0 16 16" class="bi bi-plus" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
   ...
   <div class="col-lg-6 col-12">   
                            ...
                            <div class="card-body">
                                <h1 class=" p-4 card-title bg-secondary text-light text-center">
