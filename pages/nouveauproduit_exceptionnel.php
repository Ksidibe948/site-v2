...
<?php
       ...
       {   
            ...
            $modifications = $bdd->prepare("update total_produitexceptionnelle set montant = montant +  ? where id_entreprise = ? ");
            $modifications->execute(array(  $montant_th,$_SESSION['id']));
            
            $inserts=$bdd->prepare("INSERT INTO 
            total_produitexceptionnelle(id_entreprise,montant)
            VALUES(?,?)
             ");
            $inserts->execute(array( $_SESSION['id'], $montant_th));
           
             $insert=$bdd->prepare("INSERT INTO 
             produits_execeptionnel(id_entreprise,nom_operation,montant_th, taux_tva,numero_banque,montant_payer)
             VALUES(?,?,?,?,?,?)
              ");
             $insert->execute(array( $_SESSION['id'],$nom_operation,  $montant_th,$taux_tva, $numero_banque, $montant_payer));
            
             header('Location:index.php?page=produits_execeptionnels&id='.$_SESSION['id']);
      }                 
         ?>
...
<div class="row">
   ...
   <a style='font-size:1.2rem' class="nav-link text-info p-3  mb-1 shadow-sm bg-light "  href="index.php?page=nouveauproduit_exceptionnel&id=<?=$_SESSION['id'] ?>">
  <svg width="1.1em" height="1.1em" viewBox="0 0 16 16" class="bi bi-plus" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z"/>
  </svg>
Ajouter un nouveau produit Exceptionnel
</a>
   ...
   <div class="col-lg-6 col-12">    
         <div class="row">
                            ...
                            <div class="card-body">
                                <hr><h1 class="card-title bg-secondary text-light p-2 text-center">
                                Enregistre un Produit Non Courant
                                </h1><hr>
