...
<?php
  $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
  $req=$bdd->query("SELECT publications.id_utilisateurs, publications.fichier, publications.id_publications, publications.contenu ,DATE_FORMAT(date,  '%d/%m/%Y à %Hh%imin%ss') AS date, utilisateurs.nom,utilisateurs.id, utilisateurs.images  FROM publications,utilisateurs WHERE publications.id_utilisateurs=utilisateurs.id  ORDER BY id_publications DESC");
  ?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="row">
  ...
  <div class="collapse navbar-collapse" id="navbarNavDropdown">
     <ul  class="navbar-nav mr-auto mt-2 mb-5 mt-lg-0">
      <li class="nav-item ">
      <a   class="nav-link active " style='  font-size:1.2rem'  href="index.php?page=utilisateurs">
  ...
  <div class="container">
              ...
              <div class="row">
                  <?php
                  while($reponses=$req->fetch())
                  { ?>
              
              <div class="col-lg-6">
               <a class="nav-link text-dark" href="index.php?page=les_articles&id=<?=$reponses['id_publications'] ?>"> 
                 <div class="card mb-3 shadow-lg p-1 bg-white" style="max-width:600px;">
                  ...
                  <div class="card-body text-left"> 
                   <?=$reponses['date'] ?>
                  <p class="card-text "><?= substr($reponses['contenu'],0,45) ?><span style="color:blue"></span></p>
                  <div class='imagess'>
                  <?="<img src='".$reponses['fichier']."' class='card-img-top  pt-2 pb-2  mr-2 mt-2'   alt=''>" ;?> 
                  </div>
                 </div>
                 </div>           
                  </a>
