...
if (isset($_GET['id'])) {
 $getid=$_GET['id'];
 $select=$bdd->prepare('SELECT * FROM banque WHERE id=?');
 $select->execute(array( $getid));
 $element_banque=$select->fetch();
...
<?php
       ...
       {    
             ...
             $req=$bdd->prepare('UPDATE banque SET nom_banque=? WHERE id=?');
             $req->execute(array($nom_banque,$getid));
             header('Location:visteurs.php?page=banque&id='.$getid);

             $req=$bdd->prepare('UPDATE banque SET montant=? WHERE id=?');
             $req->execute(array($montant,$getid));
             header('Location:visteurs.php?page=banque&id='.$getid);

             $req=$bdd->prepare('UPDATE banque SET frais=? WHERE id=?');
             $req->execute(array($frais,$getid));
             header('Location:visteurs.php?page=banque&id='.$getid);

             $req=$bdd->prepare('UPDATE banque SET taux_tva=? WHERE id=?');
             $req->execute(array($taux_tva,$getid));
             header('Location:visteurs.php?page=banque&id='.$getid);

             $req=$bdd->prepare('UPDATE banque SET adresse=? WHERE id=?');
             $req->execute(array($adresse,$getid));
             header('Location:visteurs.php?page=banque&id='.$getid);
             $req=$bdd->prepare('UPDATE banque SET numero=? WHERE id=?');
             $req->execute(array($numero,$getid));
             header('Location:visteurs.php?page=banque&id='.$getid);
                                 ...
                                 if (isset($_FILES['monfichier']) AND $_FILES['monfichier']['error'] == 0)
                                {
                                        ...
                                        if ($_FILES['monfichier']['size'] <= 1000000)
                                        {
                                                ...
                                                if (in_array($extension_upload, $extensions_autorisees))
                                                {
                                                        ...
                                                        $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
                                                        $req=$bdd->prepare('UPDATE  banque SET photo=? WHERE id=?');
                                                        $req->execute(array( $image,$getid));
                                                        header('Location:visteurs.php?page=banque&id='.$getid);
                                                }
                                        }
                                }else {
...
if ($_GET['id']) {
    $getid=$_GET['id'];
    $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
    $select=$bdd->prepare('SELECT * FROM banque WHERE id=? ORDER BY id DESC');
    $select->execute(array($_GET['id']));
    $resultat=$select->fetch();
    $montant_tva=$resultat['frais']*$resultat['taux_tva']/100;
    $commission_bancaire=$resultat['frais']+$montant_tva;
    $montant_ala_banque=$resultat['montant']- $commission_bancaire;
}
...
<div class="row">
     ...
     <div class="overflow-auto" style='width:72%;
       height:800px;'>
        ...
        <a  class="nav-link  p-3  mb-1 shadow-sm bg-light  bg-light " style="font-size:1.3rem; " href="visteurs.php?page=comptabilité_generale&id=<?=$_SESSION['id'] ?>">
    ...
    <a style="font-size:1.3rem;color:#A41FDE " class="nav-link p-3  mb-1 shadow-sm bg-light bg-light y bg-light " href="visteurs.php?page=banque&id=<?=$_SESSION['id'] ?>">
   ...
   <a style='margin-left:15px ;' class="nav-link p-3  mb-1 shadow-sm bg-light text-success " href="visteurs.php?page=modifier_banque&id=<?=$element_banque['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M13.498.795l.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
  </svg> Modification de   <?=$element_banque['nom_banque'] ?></a>
...
<!-- la partie inscription -->
                            ...
                            <div class="card-body">
                                <h1 class="card-title bg-secondary text-light p-4 text-center">
                                Modifier
                                </h1><hr>
                                <div class="row">
                                    <div class="col-lg-3"></div>
                                    <div class="col-lg-6">
                                    <?="<img src='".$element_banque['photo']."' class='card-img' alt=''>" ;?>
                                    </div>
                                </div><hr>
                              <form action="" method="POST" enctype="multipart/form-data">
                              <div class="form-group row">
                     
                               <label for="inputPassword" class="col-lg-3 col-form-label"><H6>Nom de la banque</h6></label>
                              <div class="col-lg-6">
                              <input type="text" value='<?=  $element_banque['nom_banque'] ?>' name='nom_banque'class="form-control" id="inputCity " required placeholder="  Ex: BMC ">
                              </div>
                            </div>
                            <div class="form-group row">
                     
                               <label for="inputPassword" class="col-lg-3 col-form-label"><H6>Adresse de la Banque</h6></label>
                              <div class="col-lg-6">
                              <input type="text" value='<?=  $element_banque['adresse'] ?>'   name='adresse'class="form-control" id="inputCity " required  placeholder="  Ex: BMC ">
                              </div>
                            </div>
                            <div class="form-group row">
                     
                               <label for="inputPassword" class="col-lg-3 col-form-label"><H6>Numéro de la banque</h6></label>
                              <div class="col-lg-6">
                              <input type="number" value='<?=  $element_banque['numero'] ?>'   name='numero' class="form-control" id="inputCity" required placeholder="  Ex:+212 0640637157 ">
                              </div> 
                            </div>
                            <div class="form-group row">
                     
                             <label for="inputPassword" class="col-lg-3 col-form-label"><H6>Email de la banque</h6></label>
                            <div class="col-lg-6">
                            <input type="text" value='<?=  $element_banque['email'] ?>'  name='email' class="form-control" id="inputCity"  placeholder="  Ex:bmc@gmail.com">
                            </div> 
                          </div>
                            <div class="form-group row">
                     
                               <label for="inputPassword" class="col-lg-3 col-form-label"><H6>Photo de la banque</h6></label>
                              <div class="col-lg-6">
                              <input type="file" value='<?=  $element_banque['photo'] ?>'  class="form-control" id="inputCity " name='monfichier'  class="form-control mr-5" id="inputCity" >
                              </div> 
                            </div>
                            <div class="form-group row">
                     
                               <label for="inputPassword" class="col-lg-3 col-form-label"><H6>Montant de depôt</h6></label>
                              <div class="col-lg-6">
                              <input type="number" value='<?=  $element_banque['montant'] ?>'   name='montant' class="form-control" id="inputCity " required placeholder="  Ex: 1000000 ">
                              </div>
                            </div>
                                 
                            <div class="form-group row">
                     
                               <label for="inputPassword" class="col-lg-3 col-form-label"><H6>Frais des opérations </h6></label>
                              <div class="col-lg-6">
                              <input type="number" value='<?=  $element_banque['frais'] ?>'   name="frais"   class="form-control" id="inputCity "  placeholder="  Ex: 100 ">
                              </div>
                            </div>
                            
                            <div class="form-group row">
                     
                               <label for="inputPassword" class="col-lg-3 col-form-label"><H6>TVA bancaire </h6></label>
                              <div class="col-lg-6">
                              <select id="inputState" value='<?=  $element_banque['taux_tva'] ?>'  name="taux_tva" class="form-control">
                                    <option>10</option>
                                </select>
                              </div>
                            </div>
