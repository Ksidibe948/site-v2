...
<?php
 if (isset($_GET['id'])) 
 {
  ...
  $select=$bdd->prepare('SELECT* FROM   achats_immobilisation  WHERE id=?');
 ...
 <?php 
       ...
       ) 
       {
        $getid=$_GET['id'];
         ...
         $modification->execute(array($modifications_bq,$condition_payement ));
 
         $req=$bdd->prepare('UPDATE  achats_immobilisation SET marchandise=? WHERE id=?');
          $req->execute(array($marchandise,$getid));
          header('Location:index.php?page=detail_materiel&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE  achats_immobilisation SET id_client=? WHERE id=?');
          $req->execute(array($numero_client,$getid));
          header('Location:index.php?page=detail_materiel&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE  achats_immobilisation SET quantite=? WHERE id=?');
          $req->execute(array($quantite,$getid));
          header('Location:index.php?page=detail_materiel&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE  achats_immobilisation SET prix=? WHERE id=?');
          $req->execute(array($prix,$getid));
          header('Location:index.php?page=detail_materiel&id='.$_GET['id']);
               
          $req=$bdd->prepare('UPDATE  achats_immobilisation SET reduction=? WHERE id=?');
          $req->execute(array($reduction,$getid));
          header('Location:index.php?page=detail_materiel&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE  achats_immobilisation SET taux_reduction=? WHERE id=?');
          $req->execute(array($taux_reduction,$getid));
          header('Location:index.php?page=detail_materiel&id='.$_GET['id']);
                       
          $req=$bdd->prepare('UPDATE  achats_immobilisation SET taux_exompte=? WHERE id=?');
          $req->execute(array($taux_exompte,$getid));
          header('Location:index.php?page=detail_materiel&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE  achats_immobilisation SET montant_transport=? WHERE id=?');
          $req->execute(array($montant_transport,$getid));
          header('Location:index.php?page=detail_materiel&id='.$_GET['id']);
                                 
          $req=$bdd->prepare('UPDATE  achats_immobilisation SET taux_tva=? WHERE id=?');
          $req->execute(array($taux_tva,$getid));
          header('Location:index.php?page=detail_materiel&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE  achats_immobilisation SET montant_emballage=? WHERE id=?');
          $req->execute(array($montant_emballage,$getid));
          header('Location:index.php?page=detail_materiel&id='.$_GET['id']);
                                           
          $req=$bdd->prepare('UPDATE  achats_immobilisation SET montant_avance=? WHERE id=?');
          $req->execute(array($montant_avance,$getid));
          header('Location:index.php?page=detail_materiel&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE  achats_immobilisation SET monyen_payement=? WHERE id=?');
          $req->execute(array($condition_payement,$getid));
          header('Location:index.php?page=detail_materiel&id='.$_GET['id']);
               
          $req=$bdd->prepare('UPDATE  achats_immobilisation SET montant_payement=? WHERE id=?');
          $req->execute(array($montant_payer,$getid));
          header('Location:index.php?page=detail_materiel&id='.$_GET['id']);
                    
          $req=$bdd->prepare('UPDATE  achats_immobilisation SET systeme=? WHERE id=?');
          $req->execute(array($systeme,$getid));
          header('Location:index.php?page=detail_materiel&id='.$_GET['id']);
                              
          $req=$bdd->prepare('UPDATE  achats_immobilisation SET duree=? WHERE id=?');
          $req->execute(array($duree,$getid));
          header('Location:index.php?page=detail_materiel&id='.$_GET['id']);

                            
          $req=$bdd->prepare('UPDATE  achats_immobilisation SET mois=? WHERE id=?');
          $req->execute(array($mois,$getid));
          header('Location:index.php?page=detail_materiel&id='.$_GET['id']);
                              
          $req=$bdd->prepare('UPDATE  achats_immobilisation SET frais_accesoire=? WHERE id=?');
          $req->execute(array($frais_accessoire,$getid));
          header('Location:index.php?page=detail_materiel&id='.$_GET['id']);
...
<div class="row">
  ...
  <div class="overflow-auto" style='width:70%;
    height:1000px;'>
     <ul  class="navbar-nav pl-3 mr-auto mt-3 mb-5 mt-lg-0">
      <li class="nav-item ">
      <a   class="nav-link  " style='  font-size:1.2rem'  href="index.php?page=utilisateurs">
     ...
     <path fill-rule="evenodd" d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8zm7.5-6.923c-.67.204-1.335.82-1.887 1.855A7.97 7.97 0 0 0 5.145 4H7.5V1.077zM4.09 4H2.255a7.025 7.025 0 0 1 3.072-2.472 6.7 6.7 0 0 0-.597.933c-.247.464-.462.98-.64 1.539zm-.582 3.5h-2.49c.062-.89.291-1.733.656-2.5H3.82a13.652 13.652 0 0 0-.312 2.5zM4.847 5H7.5v2.5H4.51A12.5 12.5 0 0 1 4.846 5zM8.5 5v2.5h2.99a12.495 12.495 0 0 0-.337-2.5H8.5zM4.51 8.5H7.5V11H4.847a12.5 12.5 0 0 1-.338-2.5zm3.99 0V11h2.653c.187-.765.306-1.608.338-2.5H8.5zM5.145 12H7.5v2.923c-.67-.204-1.335-.82-1.887-1.855A7.97 7.97 0 0 1 5.145 12zm.182 2.472a6.696 6.696 0 0 1-.597-.933A9.268 9.268 0 0 1 4.09 12H2.255a7.024 7.024 0 0 0 3.072 2.472zM3.82 11H1.674a6.958 6.958 0 0 1-.656-2.5h2.49c.03.877.138 1.718.312 2.5zm6.853 3.472A7.024 7.024 0 0 0 13.745 12H11.91a9.27 9.27 0 0 1-.64 1.539 6.688 6.688 0 0 1-.597.933zM8.5 12h2.355a7.967 7.967 0 0 1-.468 1.068c-.552 1.035-1.218 1.65-1.887 1.855V12zm3.68-1h2.146c.365-.767.594-1.61.656-2.5h-2.49a13.65 13.65 0 0 1-.312 2.5zm2.802-3.5h-2.49A13.65 13.65 0 0 0 12.18 5h2.146c.365.767.594 1.61.656 2.5zM11.27 2.461c.247.464.462.98.64 1.539h1.835a7.024 7.024 0 0 0-3.072-2.472c.218.284.418.598.597.933zM10.855 4H8.5V1.077c.67.204 1.335.82 1.887 1.855.173.324.33.682.468 1.068z"/>
      ...
      <a  class="nav-link   " style="font-size:1.2rem"  href="index.php?page=comptabilite&id=<?=$_SESSION['id'] ?>">
     ...
     <a  class="nav-link " style="font-size:1.2rem; " href="index.php?page=comptabilité_generale&id=<?=$_SESSION['id'] ?>">
 ...
 <a style="font-size:1.2rem;" class="nav-link  " href="index.php?page=banque&id=<?=$_SESSION['id'] ?>">
 <?php
...
<hr>
<a   class="nav-link  " style="font-size:1.2rem;" href="index.php?page=capital&id=<?=$_SESSION['id'] ?>">
...
<a  class="nav-link  " style="font-size:1.2rem;  " href="index.php?page=actionnaires&id=<?=$_SESSION['id'] ?>">
<?php
...
<a class="nav-link  " style="font-size:1.2rem; "   href="index.php?page=clients&id=<?=$_SESSION['id'] ?>">              
...
<a  class="nav-link " style="font-size:1.2rem; " href="index.php?page=fournisseurs&id=<?=$_SESSION['id'] ?>">              
...
<a  class="nav-link  " style="font-size:1.3rem; "  href="index.php?page=factures&id=<?=$_SESSION['id'] ?>">              
<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
 ...
 Factures
  ...
  <a  class="nav-link  " style="font-size:1.2rem; "  href="index.php?page=facturesachats&id=<?=$_SESSION['id'] ?>">              
<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M14.5 3h-13a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
  <path fill-rule="evenodd" d="M5 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 5 8zm0-2.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zm0 5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5z"/>
  <circle cx="3.5" cy="5.5" r=".5"/>
...
<a  class="nav-link  " style="font-size:1.2rem;   "  href="index.php?page=achats&id=<?=$_SESSION['id'] ?>">              
<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M14.5 3h-13a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
  <path fill-rule="evenodd" d="M5 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 5 8zm0-2.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zm0 5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5z"/>
  <circle cx="3.5" cy="5.5" r=".5"/>
 ...
 Factures Achats de Marchandises <span style='color:green'> ( <?=$stok_marchandise?> )</span>
  ...
  <?php
      if ($stok_materiel>0) {
        ?>
<a  class="nav-link  " style="font-size:1.2rem; " href="index.php?page=achat_immobilisation&id=<?=$_SESSION['id'] ?>">
<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M14.5 3h-13a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
  <path fill-rule="evenodd" d="M5 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 5 8zm0-2.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zm0 5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5z"/>
  <circle cx="3.5" cy="5.5" r=".5"/>
...
<a style="font-size:1.2rem;color:#A41FDE;" class="nav-link" href="index.php?page=modifier_achat_immobilisation&id=<?=$_GET['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
           <path fill-rule="evenodd" d="M13.498.795l.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
           </svg> Modifier la facture de <?= $donnees['marchandise'] ?></a>
     
  <a style='font-size:1.2rem ;' class="nav-link  " href="index.php?page=nouvelle_fature_achat_immobilisation&id=<?=$_SESSION['id'] ?>">
  ...
  ?>
  <a  class="nav-link  " style="font-size:1.2rem; "  href="index.php?page=autres_charges&id=<?php $_SESSION['id'] ?>">              
 ...
 Factures de autre charges <span style='color:green'>( <?=$autres_charges?>)</span> 
  ...
  <a  class="nav-link  " style="font-size:1.2rem; " href="index.php?page=facturesventes&id=<?=$_SESSION['id'] ?>">              
<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M14.5 3h-13a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
  <path fill-rule="evenodd" d="M5 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 5 8zm0-2.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zm0 5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5z"/>
  <circle cx="3.5" cy="5.5" r=".5"/>
 ...
 Factures Ventes 
  ...
  <?php
      if ($vente_materiels>0) {
        ?>
<a  class="nav-link  " style="font-size:1.1.2rem; "  href="index.php?page=ventes_materiels&id=<?=$_SESSION['id'] ?>">              
<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M14.5 3h-13a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
  <path fill-rule="evenodd" d="M5 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 5 8zm0-2.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zm0 5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5z"/>
  <circle cx="3.5" cy="5.5" r=".5"/>
...
<?php
      if ($vente_marchandise>0) {
        ?>
  <a  class="nav-link   " style="font-size:1.1.2rem; "  href="index.php?page=vente_marchandise&id=<?=$_SESSION['id'] ?>">              
<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M14.5 3h-13a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
  <path fill-rule="evenodd" d="M5 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 5 8zm0-2.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zm0 5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5z"/>
  <circle cx="3.5" cy="5.5" r=".5"/>
 ...
 Factures Ventes de marchandises <span style='color:green'><?= $vente_marchandise?></span> 
  ...
  <a  class="nav-link  " style="font-size:1.2rem; "  href="index.php?page=autres_produits&id=<?=$_SESSION['id'] ?>">              
  <?php
 ...
 Factures de autres produits <span style='color:green'> ( <?=$autres_produits?>)</span>
   ...
   <a  class="nav-link  " style="font-size:1.2rem;" href="index.php?page=Stocks&id=<?=$_SESSION['id'] ?>"> 
...
<?php
      if ($stok_marchandise>0) {
        ?>
       <a  class="nav-link   " style="font-size:1.1.2rem;"href="index.php?page=stocks_marchandise&id=<?=$_SESSION['id'] ?>"> 
        <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-cart3" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        <path fill-rule="evenodd" d="M0 1.5A.5.5 0 0 1 .5 1H2a.5.5 0 0 1 .485.379L2.89 3H14.5a.5.5 0 0 1 .49.598l-1 5a.5.5 0 0 1-.465.401l-9.397.472L4.415 11H13a.5.5 0 0 1 0 1H4a.5.5 0 0 1-.491-.408L2.01 3.607 1.61 2H.5a.5.5 0 0 1-.5-.5zM3.102 4l.84 4.479 9.144-.459L13.89 4H3.102zM5 12a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm7 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm-7 1a1 1 0 1 0 0 2 1 1 0 0 0 0-2zm7 0a1 1 0 1 0 0 2 1 1 0 0 0 0-2z"/>
       </svg>
 ...
 <?php
      if ($stok_materiel>0) {
        ?>
        <a  class="nav-link  " style="font-size:1.1.2rem;" href="index.php?page=stocks_materiels&id=<?=$_SESSION['id'] ?>"> 
        <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-cart3" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
      <path fill-rule="evenodd" d="M0 1.5A.5.5 0 0 1 .5 1H2a.5.5 0 0 1 .485.379L2.89 3H14.5a.5.5 0 0 1 .49.598l-1 5a.5.5 0 0 1-.465.401l-9.397.472L4.415 11H13a.5.5 0 0 1 0 1H4a.5.5 0 0 1-.491-.408L2.01 3.607 1.61 2H.5a.5.5 0 0 1-.5-.5zM3.102 4l.84 4.479 9.144-.459L13.89 4H3.102zM5 12a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm7 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm-7 1a1 1 0 1 0 0 2 1 1 0 0 0 0-2zm7 0a1 1 0 1 0 0 2 1 1 0 0 0 0-2z"/>
    </svg>
...
<a  class="nav-link  "  style="font-size:1.2rem;" href="index.php?page=journal&id=<?=$_SESSION['id'] ?>"> 
 ...
 <?php
      if ($stok_materiel>0) {
        ?>
<a  class="nav-link  " style="font-size:1.2rem;" href="index.php?page=amortissement&id=<?=$_SESSION['id'] ?>"> 
<svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-graph-down" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
 ...
 Amortissements <span style='color:green'>( <?= $stok_materiel?>)</span> 
  ...
  ?>
<a  class="nav-link  "style="font-size:1.2rem;"  href="index.php?page=balance&id=<?=$_SESSION['id'] ?>">
...
<a  class="nav-link " style="font-size:1.2rem;" href="index.php?page=Bilan&id=<?=$_SESSION['id'] ?>">
 ...
 <a  class="nav-link  " style="font-size:1.2rem;" href="index.php?page=cpc&id=<?=$_SESSION['id'] ?>"> 
 <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-file-text" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M4 0h8a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V2a2 2 0 0 1 2-2zm0 1a1 1 0 0 0-1 1v12a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H4z"/>
  <path fill-rule="evenodd" d="M4.5 10.5A.5.5 0 0 1 5 10h3a.5.5 0 0 1 0 1H5a.5.5 0 0 1-.5-.5zm0-2A.5.5 0 0 1 5 8h6a.5.5 0 0 1 0 1H5a.5.5 0 0 1-.5-.5zm0-2A.5.5 0 0 1 5 6h6a.5.5 0 0 1 0 1H5a.5.5 0 0 1-.5-.5zm0-2A.5.5 0 0 1 5 4h6a.5.5 0 0 1 0 1H5a.5.5 0 0 1-.5-.5z"/>
</svg>
 CPC
     ...
     <div class="col-lg-8">
                            ...
                            <div class="col-lg-12">
                            <div class="card shadow-lg w-100 bg-light" style="width: 18rem; ">
                            <div class="card-body">
                                <hr><h1 class="card-title text-center">
                               
                              Modifier la facture de <?= $donnees['marchandise'] ?>
                                </h1><hr>
                                <form action="" method="POST" enctype="multipart/form-data">
                         

                              <div class="form-group row">
                              <label for="inputPassword" class="col-sm-3 col-form-label"> Fournisseur  :  </label>
                              <div class="col-sm-3">
                            ...
                            <div class="form-group row">
                              ...
                              <div class="col-sm-3">
                              <input type="text" value='<?= $donnees['marchandise'] ?>'  name='marchandise'class="form-control" id="inputCity" required placeholder="EX: Tomate">
                              </div>
                              <label for="inputPassword" class="col-sm-1 col-form-label">Quantité  :  </label>
                              <div class="col-sm-2">
                              <input type="number" value='<?= $donnees['quantite'] ?>'  name='quantite'class=" form-control" id="inputCity " required placeholder="EX: 10 pièces">
                              </div>
                              <label for="inputPassword" class="col-sm-1 col-form-label">Prix  :  </label>
                              <div class="col-sm-2">
                              <input type="number" value='<?= $donnees['prix'] ?>'  name='prix'class="form-control" id="inputCity" required placeholder="Mettez le prix">
                              </div>
                            </div><hr>

                            <div class="form-group row">
                              <label for="inputPassword" class="col-sm-3 col-form-label">Réductions   :  </label>
                              <div class="col-sm-3">
                              <select id="inputState" value='<?= $donnees['reduction'] ?>' name="reduction" class="form-control mr-5">
                              ...
                              <label for="inputPassword" class="col-sm-1 col-form-label">Taux  :  </label>
                              <div class="col-sm-3">
                              <select id="inputState" value='<?= $donnees['taux_reduction'] ?>' name="taux_reduction" class="form-control ">
                                    <option>0</option>
                                    <option>1</option>
                                    <option>3</option>
                                    <option>3</option>
                              ...
                              <div class="form-group row">
                              <label for="inputPassword" class="col-sm-3 col-form-label">Exompte  :  </label>
                              <div class="col-sm-3">
                              <select id="inputState" value='<?= $donnees['taux_exompte'] ?>' name="taux_exompte" class="form-control">
                                    <option>0</option>
                                    <option>1</option>
                                    <option>3</option>
                                    <option>3</option>
                            ...
                            <div class="form-group row">
                              <label for="inputPassword" class="col-sm-3 col-form-label">Transport  :  </label>
                              <div class="col-sm-3">
                              <input type="number" value='<?= $donnees['montant_transport'] ?>' name='montant_transport'class="form-control" id="inputCity "  placeholder="  Ex: 100 ">
                              </div>
                            </div><hr>
                            
                            <div class="form-group row">
                              <label for="inputPassword" class="col-sm-3 col-form-label">TVA  :  </label>
                              <div class="col-sm-3">
                              <select id="inputState" value='<?= $donnees['taux_tva'] ?>' name="taux_tva" class="form-control">
                                    <option>0</option>
                                    <option>10</option>
                                    <option>16</option>
                                    <option>20</option>
                            ...
                            <div class="form-group row">
                              <label for="inputPassword" class="col-sm-3 col-form-label">Emballage  :  </label>
                              <div class="col-sm-3">
                              <input type="number"value='<?= $donnees['montant_emballage'] ?>'  name='montant_emballage'class="form-control" id="inputCity "  placeholder="  Ex: 100 ">
                              </div>
                            </div><hr>
                                       
                            <div class="form-group row">
                              <label for="inputPassword" class="col-sm-3 col-form-label">Avance  :  </label>
                              <div class="col-sm-3">
                              <input type="number" value='<?= $donnees['montant_avance'] ?>'  name='montant_avance'class="form-control" id="inputCity "  placeholder="  Ex: 100 ">
                              </div>
                            </div><hr>
                                       
                            <div class="form-group row">
                            <label for="inputPassword" class="col-sm-3 col-form-label">Banque   </label>
                              <div class="col-sm-3">
                            ...
                            <?php
                              ...
                              <label for="inputPassword" class="col-sm-3 col-form-label">Montant payer  :  </label>
                              <div class="col-sm-3">
                              <input type="number" value='<?= $donnees['montant_payement'] ?>'  name='montant_payer'class="form-control" id="inputCity "  placeholder="  Ex: 100 ">
                              </div>
                            </div><hr>
...
<div class="col-lg-12 text-center"><h5>Amortissement</h5></div>
<div class="col-lg-12">
<div class="alert alert-warning alert-dilgissible fade show text-center" role="alert">
  <strong>Amortissement !  </strong>Vous pouvez vous modifier ces informations!
</div>     
</div><hr>
                                
<div class="form-group row">
<div class="col-lg-3"></div>
<label for="inputPassword" class="col-lg-4 col-form-label">Frais accessoires :  </label>
<div class="col-lg-3">
<input type="number" value='<?= $donnees['frais_accessoire'] ?>'  name='frais_accesoire'class="form-control" id="inputCity "  placeholder="frais de Douane, installation,... ">
</div>
</div><hr>
<div class="form-group row">

<label for="inputPassword" class="col-lg-4 col-form-label ml-1">Mois de mis en service:</label>
 <div class="col-lg-3">
 ...
 <div class="form-group row">
<label for="inputPassword" class="col-lg-3 col-form-label"> Système</label>
<div class="col-lg-3">
<select id="inputState" value='<?= $donnees['systeme'] ?>' name="systeme" class="form-control mr-5">
   <option>Lineaire</option>
  <option>Dégréssif</option>
</select>
</div>
<label for="inputPassword"  class="col-lg-3 col-form-label">Durées de vie:</label>
<div class="col-lg-3">
<input type="number" value='<?= $donnees['duree'] ?>'  name='duree'class=" form-control" id="inputCity " required placeholder="EX: 5 ans">
</div>
</div><hr>
