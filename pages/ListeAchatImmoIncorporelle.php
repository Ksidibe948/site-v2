...
$bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   ...
   <?php
$bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $list=$bdd->prepare('SELECT id, marchandiseS, quantiteS,  DATE_FORMAT(date,"%d/%m/%Y à %Hh%imin%ss") AS date FROM stocksimmoincorporelle where id_entrepriseS=? ORDER BY id DESC');
            ...
            <?php 
             if ($resultat['quantiteS']>0) {
               ?>
                ...
                <a class="nav-link ligh text-secondary bg-light" href="visteurs.php?page=NouvelleFactureCessionImmoIncorporelle&id=<?=$resultat['id'] ?>">
