...
<?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $ventes_marchandise=$bdd->prepare("SELECT
        ventes.id,
        ventes.id_client,
   ...
   FROM   ventes,clients WHERE   ventes.id_client=clients.id AND   ventes. id_client=? ORDER BY id DESC");
  $ventes_marchandise->execute(array($_GET['id']));
   $compte= $ventes_marchandise->rowcount();
 ?>
...
<?php
   ...
   FROM    vente_materiels,clients WHERE    vente_materiels.id_client=clients.id AND  vente_materiels. id_client=? ORDER BY id DESC");
...
<?php
  ...
  <svg width="1.2em" height="1.2em" viewBox="0 0 16 16" class="bi bi-people" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
   ...
   <a style="font-size:1.2rem;" class="nav-link  text-success" href="index.php?page=modifier_clients&id=<?=$resultat['id']?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" d="M13.498.795l.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
                </svg> Modifier Client <?= $resultat['nom'] ?></a>
                <a  style="font-size:1.2rem;" class="nav-link  text-danger" href="index.php?page=supprimer_clients&id=<?=$resultat['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-x" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
               <path fill-rule="evenodd" d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708z"/>
               </svg> Supprimer Client <?= $resultat['nom'] ?></a>
   <a style='font-size:1.1rem' class="nav-link  text-info"  href="index.php?page=nouveau_clients&id=<?=$_SESSION['id'] ?>">
  ...
  <a  class="nav-link " style="font-size:1.2rem;" href="index.php?page=Bilan&id=<?=$_SESSION['id'] ?>">
   ...
   CPC
    ...
    <div class="container">
    <div class="col-lg-12">
    <div class="row  mt-4 pb-4  pr-2 pt-2" >
     <div class="col-12">
    ...
    Date de création du compte : le<?= $resultat['date'] ?><br>
   Nom: <?= $resultat['nom'] ?><br>
   Téléphone: <?= $resultat['telephone'] ?><br>
   Adresse: <?= $resultat['adresse'] ?><br><br>

   <span style='font-size:1.5rem'>Nombre de marchandise acheter (<?=$compptemats?>)</span> <br>
   <span style='font-size:1.5rem'>Nombre de materiel acheter (<?=  $compptemat?>)</span> 
    </div>
    </div>
    </div>
    </div>
...
<?php
   while($donnees= $ventes_marchandise->fetch()){
       ...
       <div class="container">
        <div class="row mb-2">
         <div class="col-12">
         <a  class="nav-link  text-dark"  href="index.php?page=detail_vente_marchandise&id=<?=$donnees['id'] ?>">
...
<?php
   while($donnees=$vtm_materiels->fetch()){
        $montant=$donnees['quantite']*$donnees['prix'];
        $reductions=$donnees['taux_reduction']/100;
        $montant_reduction=$montant*$reductions;
        $net_commercial=$montant-$montant_reduction;
       ...
       <div class="container">
        <div class="row mb-2">
         <div class="col-12">
          <div class="card shadow-lg w-100"><hr>
          <a  class="nav-link text-dark " style="font-size:1.2rem;" href="index.php?page=detail_vente_materiel&id=<?=$donnees['id'] ?>" > 
            <div class="row mr-5">
            <div class="col-12 text-right">
                 <?= $donnees['date'] ?>
              </div>
