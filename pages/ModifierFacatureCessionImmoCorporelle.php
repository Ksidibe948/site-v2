...
?>
<?php
 $req=$bdd->prepare('SELECT * FROM  CessionImmoCorporelle WHERE id=?');
...
?>
 <?php 
       ...
       if (isset($_POST['marchandise'])
        ...
        $modifications_bq = $reponse['montant_payement'] - $montant_payer; 
        ...
        $ventess = $bdd->prepare('UPDATE CessionImmoCorporelle set quantite= ? where id= ? ');
        $ventess->execute(array($nouvelle_valeur, $_GET['id']));
      
        $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');

        $req=$bdd->prepare('UPDATE CessionImmoCorporelle SET marchandise=? WHERE id=?');
        $req->execute(array($marchandise,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoCorporelle&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE CessionImmoCorporelle SET id_client=? WHERE id=?');
        $req->execute(array($numero_client,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoCorporelle&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE CessionImmoCorporelle SET quantite=? WHERE id=?');
        $req->execute(array($quantite,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoCorporelle&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE CessionImmoCorporelle SET prix=? WHERE id=?');
        $req->execute(array($prix,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoCorporelle&id='.$_GET['id']);
             
        $req=$bdd->prepare('UPDATE CessionImmoCorporelle SET reduction=? WHERE id=?');
        $req->execute(array($reduction,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoCorporelle&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE CessionImmoCorporelle SET taux_reduction=? WHERE id=?');
        $req->execute(array($taux_reduction,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoCorporelle&id='.$_GET['id']);
                     
        $req=$bdd->prepare('UPDATE CessionImmoCorporelle SET taux_exompte=? WHERE id=?');
        $req->execute(array($taux_exompte,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoCorporelle&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE CessionImmoCorporelle SET montant_transport=? WHERE id=?');
        $req->execute(array($montant_transport,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoCorporelle&id='.$_GET['id']);
                               
        $req=$bdd->prepare('UPDATE CessionImmoCorporelle SET taux_tva=? WHERE id=?');
        $req->execute(array($taux_tva,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoCorporelle&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE CessionImmoCorporelle SET montant_emballage=? WHERE id=?');
        $req->execute(array($montant_emballage,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoCorporelle&id='.$_GET['id']);
                                         
        $req=$bdd->prepare('UPDATE CessionImmoCorporelle SET montant_avance=? WHERE id=?');
        $req->execute(array($montant_avance,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoCorporelle&id='.$_GET['id']);
        
        $req=$bdd->prepare('UPDATE CessionImmoCorporelle SET monyen_payement=? WHERE id=?');
        $req->execute(array($condition_payement,$getid));
        header('Location:visteurs.php?page=DetailCessionImmoCorporelle&id='.$_GET['id']);
             
        $req=$bdd->prepare('UPDATE CessionImmoCorporelle SET montant_payement=? WHERE id=?');
...
$bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   ...
   ?>
        <?php
   $bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $vente_marchandise=$bdd->prepare('SELECT * FROM CessionImmoCorporelle where id_entreprise=? ORDER BY id DESC');
...
<div class="row">
 ...
 <a  class="nav-link  p-3  mb-1 shadow-sm bg-light "  style="font-size:1.2rem;color:#A41FDE" href="visteurs.php?page=CessionImmoCorporelle&id=<?=$_SESSION['id'] ?>"> 
  <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M14.5 3h-13a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
  <path fill-rule="evenodd" d="M5 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 5 8zm0-2.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zm0 5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5z"/>
  <circle cx="3.5" cy="5.5" r=".5"/>
...
Cessions Immobilisations Corporelles
 ...
 <a style=' font-size:1.1rem;;' class="nav-link  p-3  mb-1 shadow-sm bg-light  text-success  " href="visteurs.php?page=ModifierFacatureCessionImmoCorporelle&id=<?= $reponse['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M13.498.795l.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001zm-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708l-1.585-1.585z"/>
  </svg> Modifier la facute de <?= $reponse['marchandise'] ?></a>
 <a  class="nav-link p-3  mb-1 shadow-sm bg-light  bg-light "  style="font-size:1.2rem;" href="visteurs.php?page=produits&id=<?=$_SESSION['id'] ?>"> 
   ...
   <div class="col-lg-6 col-12"> 
                            ...
                            <div class="card-body">
                                <h1 class="card-title p-4 text-light bg-secondary mb-3  text-center">
                              Modifier la Facture de Cession <?= $reponse['marchandise']?>
                                </h1><hr>
                                <form action="" method="POST" enctype="multipart/form-data">
                              <div class="form-group row">
                              <label for="inputPassword" class="col-sm-2 col-form-label"> <h6>Client: </h6> </label>
                              <div class="col-sm-3">
                            ...
                            <div class="form-group row">
                              <label for="inputPassword" class="col-sm-2 col-form-label"><h6>Produit : </h6> </label>
                              <div class="col-sm-3">
                            ...
                            <div class="form-group row">
                              ...
                              <div class="col-sm-3">
                              <input type="number" value='<?= $reponse['montant_transport']?>' name='montant_transport'class="form-control" id="inputCity "  placeholder="  Ex: 100 ">
                              </div>
                            </div>
                            <div class="form-group row">
                            ...
                            <div class="form-group row">
                              ...
                              <div class="col-sm-3">
                              <input type="number"  name='montant_emballage' value='<?= $reponse['montant_emballage']?>' class="form-control" id="inputCity "  placeholder="  Ex: 100 ">
                              </div>
                            </div>
                            <div class="form-group row">
                              <label for="inputPassword" class="col-sm-2 col-form-label"><h6>Avance :</h6>  </label>
                              <div class="col-sm-3">
                              <input type="number"  name='montant_avance' value='<?= $reponse['montant_avance']?>'class="form-control" id="inputCity "  placeholder="  Ex: 100 ">
                              </div>
                            </div>
                            <div class="form-group row">
                              <label for="inputPassword" class="col-sm-3 col-form-label"><h6>Numéro Banque :</h6>  </label>
                              <div class="col-sm-3">
                            ...
                            <?php
                              ...
                              <label for="inputPassword" class="col-sm-3 col-form-label"><h6>Montant payer :</h6>  </label>
                              <div class="col-sm-3">
                              <input type="number"  name='montant_payer' value='<?= $reponse['montant_payement']?>' class="form-control" id="inputCity "  placeholder="  Ex: 100 ">
                              </div>
                            </div>
                                <div class="form-group text-center">
                                    <input type="submit" class='bg-success text-white' value="Valider" name="valider" >
                                </div>
                                </form>
                            </div>
                            </div>
