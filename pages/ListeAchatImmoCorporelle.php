...
<?php
$bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $listeachat_marchandise=$bdd->prepare('SELECT id, marchandiseS, quantiteS,  DATE_FORMAT(date,"%d/%m/%Y à %Hh%imin%ss") AS date FROM stocksimmocorporelle where id_entrepriseS=? ORDER BY id DESC');
   $listeachat_marchandise->execute(array($_SESSION['id']));
   
   ?>
   <?php
$bdd=new PDO('mysql:host=localhost; dbname=w&k;charset=utf8','root','');
   $list=$bdd->prepare('SELECT id, marchandiseS, quantiteS,  DATE_FORMAT(date,"%d/%m/%Y à %Hh%imin%ss") AS date FROM stocksimmocorporelle where id_entrepriseS=? ORDER BY id DESC');
...
Cessions Immobilisations Corporelles
 ...
 <svg width="1.1em" height="1.1em" viewBox="0 0 16 16" class="bi bi-plus" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
   ...
   Liste des achats d'immobilisation Coporelles 
 </a>
            ...
            <?php 
             if ($resultat['quantiteS']>0) {
               ?>
                ...
                <a class="nav-link ligh text-secondary bg-light " href="visteurs.php?page=NouvelleFactureCessionImmoCorporelle&id=<?=$resultat['id'] ?>">
