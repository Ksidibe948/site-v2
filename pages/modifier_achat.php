...
<?php
 if (isset($_GET['id'])) 
 {
  ...
  $select=$bdd->prepare('SELECT* FROM achats WHERE id=?');
 ...
 <?php 
       ...
       ) 
       {
        $getid=$_GET['id'];
         ...
         $reqS=$bdd->prepare('UPDATE stoks_marchandise SET quantiteS=? WHERE id=?');
         $reqS->execute(array($quantite,$getid));
         header('Location:visteurs.php?page=achat&id='.$_GET['id']);
         
         $reqS=$bdd->prepare('UPDATE stoks_marchandise SET prixS=? WHERE id=?');
         $reqS->execute(array($prix,$getid));
         header('Location:visteurs.php?page=achat&id='.$_GET['id']);
              
         $reqS=$bdd->prepare('UPDATE stoks_marchandise SET id_fournisseurS=? WHERE id=?');
         $reqS->execute(array($numero_client,$getid));
         header('Location:visteurs.php?page=achat&id='.$_GET['id']);
         
         $reqS=$bdd->prepare('UPDATE stoks_marchandise SET marchandiseS=? WHERE id=?');
         $reqS->execute(array($marchandise,$getid));
         header('Location:visteurs.php?page=achat&id='.$_GET['id']);
         

          $req=$bdd->prepare('UPDATE achats SET marchandise=? WHERE id=?');
          $req->execute(array($marchandise,$getid));
          header('Location:visteurs.php?page=achat&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE achats SET id_fournisseur=? WHERE id=?');
          $req->execute(array($numero_client,$getid));
          header('Location:visteurs.php?page=achat&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE achats SET quantite=? WHERE id=?');
          $req->execute(array($quantite,$getid));
          header('Location:visteurs.php?page=achat&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE achats SET prix=? WHERE id=?');
          $req->execute(array($prix,$getid));
          header('Location:visteurs.php?page=achat&id='.$_GET['id']);
               
          $req=$bdd->prepare('UPDATE achats SET reduction=? WHERE id=?');
          $req->execute(array($reduction,$getid));
          header('Location:visteurs.php?page=achat&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE achats SET taux_reduction=? WHERE id=?');
          $req->execute(array($taux_reduction,$getid));
          header('Location:visteurs.php?page=achat&id='.$_GET['id']);
                       
          $req=$bdd->prepare('UPDATE achats SET taux_exompte=? WHERE id=?');
          $req->execute(array($taux_exompte,$getid));
          header('Location:visteurs.php?page=achat&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE achats SET montant_transport=? WHERE id=?');
          $req->execute(array($montant_transport,$getid));
          header('Location:visteurs.php?page=achat&id='.$_GET['id']);
                                 
          $req=$bdd->prepare('UPDATE achats SET taux_tva=? WHERE id=?');
          $req->execute(array($taux_tva,$getid));
          header('Location:visteurs.php?page=achat&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE achats SET montant_emballage=? WHERE id=?');
          $req->execute(array($montant_emballage,$getid));
          header('Location:visteurs.php?page=achat&id='.$_GET['id']);
                                           
          $req=$bdd->prepare('UPDATE achats SET montant_avance=? WHERE id=?');
          $req->execute(array($montant_avance,$getid));
          header('Location:visteurs.php?page=achat&id='.$_GET['id']);
          
          $req=$bdd->prepare('UPDATE achats SET monyen_payement=? WHERE id=?');
          $req->execute(array($condition_payement,$getid));
          header('Location:visteurs.php?page=achat&id='.$_GET['id']);
               
          $req=$bdd->prepare('UPDATE achats SET montant_payement=? WHERE id=?');
...
<div class="row">
   ...
   <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    ...
    Factures Achats
     ...
     <a  class="nav-link p-3  mb-1 shadow-sm bg-light bg-light bg-light "  style="font-size:1.2rem; " href="visteurs.php?page=achats&id=<?=$_SESSION['id'] ?>"> 
  <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-card-list" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  <path fill-rule="evenodd" d="M14.5 3h-13a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h13a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5zm-13-1A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-13z"/>
  <path fill-rule="evenodd" d="M5 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 5 8zm0-2.5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5zm0 5a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5z"/>
  <circle cx="3.5" cy="5.5" r=".5"/>
  ...
  Factures Achat Marchandises
  </a>
   <a style="font-size:1.2rem;" class="nav-link p-3  mb-1 shadow-sm bg-light  text-success" href="visteurs.php?page=modifier_achat&id=<?=$_GET['id'] ?>"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pen" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
